...
void SelectFileDialogOHOS::SelectFileImpl(
  ...
  auto callback = [this, multi_files](const std::string& path) {
    ...
    auto task = paths.size()
                    ? base::BindOnce(&SelectFileDialogOHOS::CompleteOpen, this,
                                     paths, multi_files)
  ...
  switch (type) {
    ...
    case SELECT_OPEN_FILE:
      FileSelectPicker::GetInstance().ShowFilePickerDialog(
          false, file_types->extensions, callback);
      break;
    case SELECT_OPEN_MULTI_FILE:
      FileSelectPicker::GetInstance().ShowFilePickerDialog(
          true, file_types->extensions, callback);
      break;
    case SELECT_SAVEAS_FILE: {
      std::string file_name = default_path.BaseName().AsUTF8Unsafe();
      std::string dir_name = default_path.DirName().AsUTF8Unsafe();
      FileSelectPicker::GetInstance().ShowSaveAsDialog(
          file_name, dir_name, file_types->extensions, callback);
      break;
    }
    case SELECT_NONE:
      NOTREACHED();
...
SelectFileDialogOHOS::ConvertJSONToStringArray(const std::string& json) {
  Json::Reader reader;
  Json::Value root;
