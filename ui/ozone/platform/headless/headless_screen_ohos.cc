...
constexpr size_t kHeadlessOhosWidthHeightSplitSize = 2;

// Parse comma-separated screen width and height.
bool ParseScreenSizeOhos(const std::string& screen_size, int* width, int* height) {
  ...
  if (width_and_height.size() != kHeadlessOhosWidthHeightSplitSize)
    return false;
...
void HeadlessScreenListener::OnCreate(OHOS::NWeb::DisplayId display_id) {
  if (headless_screen_ohos_ && task_runner_) {
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&HeadlessScreenOhos::OnDisplayCreate, headless_screen_ohos_, display_id));
  }
}
...
void HeadlessScreenListener::OnDestroy(OHOS::NWeb::DisplayId display_id) {
  if (headless_screen_ohos_ && task_runner_) {
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&HeadlessScreenOhos::OnDisplayDestroy, headless_screen_ohos_, display_id));
  }
}
...
void HeadlessScreenListener::OnChange(OHOS::NWeb::DisplayId display_id) {
  if (headless_screen_ohos_ && task_runner_) {
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&HeadlessScreenOhos::OnDisplayChange, headless_screen_ohos_, display_id));
  }
}
