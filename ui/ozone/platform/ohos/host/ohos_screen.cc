...
// system navigation bar height, work for harden only, does not work for klv.
const uint32_t kBottomAppBarHeight = 125;
  ...
  if (configRect.size() == kOhosDisplaySize) {
    dst_display.SetScaleAndBounds(
        ...
        gfx::Rect(gfx::Size(ohos_display.width,
                            ohos_display.height - kBottomAppBarHeight)));
  } else {
...
void OhosScreen::OnDisplayEventCallback(const std::string& event,
                                        int32_t display_id) {
...
void OhosScreen::OnDisplayEvent(const std::string& event, int32_t display_id) {
...
void OhosScreen::RegisterDisplayCallback() {
  auto& screenAdapter = ohos::adapter::ScreenAdapter::GetInstance();
  screenAdapter.RegisterCallback(std::bind(&OhosScreen::OnDisplayEventCallback,
