...
void OhosWindow::RegistWindowEvent() {
  WindowAdapter::GetInstance().RegistWindowEvent(
      GetWidget(),
      [this](gfx::AcceleratedWidget widget, std::shared_ptr<XCEvent> event) {
        CHECK(event);
...
void OhosWindow::OnSurfaceCreated() {
  auto task = base::BindOnce(
      [](base::WeakPtr<OhosWindow> window, gfx::AcceleratedWidget widget) {
        if (window) {
...
void OhosWindow::OnRequestClose() {
  auto task = base::BindOnce(
      [](base::WeakPtr<OhosWindow> window) {
        if (window) {
...
void OhosWindow::Applied(const gfx::Rect& origin_bounds,
                         ...
                         bool force) {
  bool origin_changed = origin_bounds.origin() != new_bounds.origin() || force;
...
uint32_t OhosWindow::DispatchEventToDelegate(
