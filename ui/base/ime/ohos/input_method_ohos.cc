...
InputMethod_AttachOptions* InputMethodOHOSCAPI::UpdateAttachOptions(
  ...
  if (!attach_options_) {
    ...
    if (!attach_options_) {
      LOG(ERROR) << "[InputMethodOHOSCAPI] create attach_options fail, attach_option is null";
      return nullptr;
    }
    LOG(INFO) << "[InputMethodOHOSCAPI] "
                 "OhAttachOptionsCreateWithRequestKeyboardReason success, reason: " << keyboard_reason;
