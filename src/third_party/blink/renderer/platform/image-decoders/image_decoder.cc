...
#include "third_party/blink/renderer/platform/image-decoders/ohos/jpge_image_decoder_ohos.h"
#include "third_party/blink/renderer/platform/image-decoders/ohos/png_image_decoder_ohos.h"
#include "third_party/blink/renderer/platform/image-decoders/ohos/webp_image_decoder_ohos.h"
#endif
...
std::unique_ptr<ImageDecoder> ImageDecoder::CreateByMimeType(
  ...
  if (mime_type == "image/jpeg" || mime_type == "image/pjpeg" ||
      mime_type == "image/jpg") {
#if BUILDFLAG(IS_OHOS)
    decoder = std::make_unique<JPGEImageDecoderOhos>(
...
#if BUILDFLAG(IS_OHOS)
    decoder = std::make_unique<PNGImageDecoderOhos>(
        alpha_option, high_bit_depth_decoding_option, color_behavior,
        max_decoded_bytes, animation_option);
#else
    decoder =
...
#if BUILDFLAG(IS_OHOS)
    decoder = std::make_unique<WEBPImageDecoderOhos>(
        alpha_option, high_bit_depth_decoding_option, color_behavior,
        max_decoded_bytes, animation_option);
#else
