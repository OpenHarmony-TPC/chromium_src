...
bool ArkWebNativeObject::FireValidCallback() {
  ...
  if (!validCallback_) {
    LOG(DEBUG) << "NativeArkWeb valid callback is nullptr";
    return false;
  }
  validCallback_();
  return true;
...
bool ArkWebNativeObject::FireLoadStartCallback() {
  if (!loadStartCallback_) {
    LOG(DEBUG) << "NativeArkWeb load start callback is nullptr";
    return false;
  }
  loadStartCallback_();
  return true;
...
bool ArkWebNativeObject::FireLoadEndCallback() {
  if (!loadEndCallback_) {
    LOG(DEBUG) << "NativeArkWeb load end callback is nullptr";
    return false;
  }
  loadEndCallback_();
  return true;
