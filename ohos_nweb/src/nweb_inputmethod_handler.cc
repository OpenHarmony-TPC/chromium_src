...
void NWebInputMethodHandler::Attach(CefRefPtr<CefBrowser> browser,
  ...
  if (show_keyboard && isAttached_) {
    ...
    config.SetTextInputType(TextInputType::TEXT);
    if (input_mode == CEF_TEXT_INPUT_MODE_NUMERIC) {
      config.SetTextInputType(TextInputType::NUMBER);
    }
    InputMethodController::GetInstance()->OnConfigurationChange(config);
...
void NWebInputMethodHandler::OnSelectionChanged(CefRefPtr<CefBrowser> browser,
                                                const CefString& text,
                                                const CefRange& selected_range) {
  InputMethodController::GetInstance()->OnSelectionChange(text.ToString16(),
                                                          selected_range.from,
                                                          selected_range.to);
}
