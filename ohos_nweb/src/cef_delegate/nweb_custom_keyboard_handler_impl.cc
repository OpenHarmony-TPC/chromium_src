...
void NWebCustomKeyboardHandlerImpl::InsertText(const std::string &text) {
  ...
  if (!isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard custom keyboard is not attached, insert text return";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::DeleteForward(int32_t length) {
  ...
  if (!isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard custom keyboard is not attached, delete forward return";
    return;
  }

  if (!browser_ || !browser_->GetHost()) {
    LOG(ERROR) << "WebCustomKeyboard delete forward failed, browser_ is nullptr!";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::DeleteBackward(int32_t length) {
  ...
  if (!isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard custom keyboard is not attached, delete backward return";
    return;
  }

  if (!browser_ || !browser_->GetHost()) {
    LOG(ERROR) << "WebCustomKeyboard delete backward failed, browser_ is nullptr!";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::SendFunctionKey(int32_t key) {
  ...
  if (!isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard custom keyboard is not attached, send function key return";
    return;
  }

  if (!browser_ || !browser_->GetHost()) {
    LOG(ERROR) << "WebCustomKeyboard send function key failed, browser_ is nullptr!";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::Attach(CefRefPtr<CefBrowser> browser, bool show_keyboard, int32_t input_flags) {
  ...
  if (isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard attach, custom keyboard is already attached, don't attach again";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::Close() {
  ...
  if (!isAttached_) {
    LOG(ERROR) << "WebCustomKeyboard close, custom keyboard is already closed, don't close again";
    return;
  }
...
void NWebCustomKeyboardHandlerImpl::CloseFromWebStateChange(WebCustomKeyboardState mode) {
  LOG(INFO) << "WebCustomKeyboard CloseFromWebStateChange, mode = " << static_cast<int>(mode);
...
bool NWebCustomKeyboardHandlerImpl::AttachFromWebStateChange(WebCustomKeyboardState mode) {
  LOG(INFO) << "WebCustomKeyboard AttachFromWebStateChange, mode = " << static_cast<int>(mode);

  if (isAttached_) {
    LOG(INFO) << "WebCustomKeyboard AttachFromWebStateChange, custom keyboard don't need reattch";
    return false;
  }
  ...
  } else if (mode == WebCustomKeyboardState::FROME_ONCONTINUE && isCloseFromOnpause_) {
    isNeedReattach = true;
