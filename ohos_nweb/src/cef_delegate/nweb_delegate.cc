...
void NWebDelegate::ExecuteAction(int64_t accessibilityId,
                                 uint32_t action, const std::map<std::string, std::string>& actionArguments) {
  auto* accessibilityManager = GetAccessibilityManager();
  if (accessibilityManager == nullptr) {
    return;
  }
  ...
  switch (aceAction) {
    ...
    case AceAction::ACTION_SET_SELECTION: {
      ...
      accessibilityManager->SetSelection(
          content::BrowserAccessibility::AXRange(node->CreatePositionForSelectionAt(start),
    ...
    case AceAction::ACTION_SET_TEXT: {
      ...
      accessibilityManager->SetSelection(
          content::BrowserAccessibility::AXRange(node->CreatePositionForSelectionAt(newText.length()),
