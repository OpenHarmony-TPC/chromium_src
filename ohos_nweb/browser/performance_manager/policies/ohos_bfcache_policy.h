...
void MaybeFlushBFCacheOnUIThread(const WebContentsProxy& contents_proxy,
  ...
  switch (memory_pressure_level) {
    case MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE:
      LOG(ERROR) << "OHOSBFCache::MaybeFlushBFCacheOnUIThread" << " The value of memory pressure level is: MEMORY_PRESSURE_LEVEL_MODERATE";
      cache_size = 1;
      break;
    case MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_CRITICAL:
      LOG(ERROR) << "OHOSBFCache::MaybeFlushBFCacheOnUIThread" << " The value of memory pressure level is: MEMORY_PRESSURE_LEVEL_CRITICAL";
      cache_size = 0;
      break;
    default:
      NOTREACHED();
  ...
  if (!navigation_controller.GetPendingEntry()) {
    LOG(ERROR) << "OHOSBFCache::MaybeFlushBFCacheOnUIThread" << " Start to prune cache is: " << cache_size;
