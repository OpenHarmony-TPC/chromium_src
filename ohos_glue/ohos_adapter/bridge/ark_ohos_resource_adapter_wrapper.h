...
public:
  ...
  bool GetRawFileData(const std::string &rawFile, size_t &len, uint8_t **dest,
                      bool isSys = false) override;

  std::shared_ptr<NWeb::OhosFileMapper>
  GetRawFileMapper(const std::string &rawFile, bool isSys = false) override;

  bool IsRawFileExist(const std::string &rawFile, bool isSys = false) override;

  bool GetRawFileLastModTime(const std::string &rawFile, uint16_t &date,
                             uint16_t &time, bool isSys = false) override;

  bool GetRawFileLastModTime(const std::string &rawFile, time_t &time,
                             bool isSys = false) override;
