...
static ArkPasteRecordVector
ArkPasteRecordVectorClassToStruct(const NWeb::PasteRecordVector &class_value) {
  ArkPasteRecordVector struct_value = {.size = class_value.size(),
  ...
  if (struct_value.size > 0) {
    ...
    for (auto it = class_value.begin(); it != class_value.end(); it++) {
      ...
      struct_value.value[count] =
          ArkPasteDataRecordAdapterCToCpp::Revert(data->ctocpp_);
      count++;
    }
  }
  return struct_value;
...
ArkPasteBoardClientAdapterWrapper::ArkPasteBoardClientAdapterWrapper(
    ArkWebRefPtr<ArkPasteBoardClientAdapter> ref)
    : ctocpp_(ref) {
}

bool ArkPasteBoardClientAdapterWrapper::GetPasteData(
    NWeb::PasteRecordVector &data) {
  if (!ctocpp_) {
    return false;
  }
...
int32_t
ArkPasteBoardClientAdapterWrapper::OpenRemoteUri(const std::string &path) {
  if (!ctocpp_) {
    return -1;
  }
...
int32_t ArkPasteBoardClientAdapterWrapper::AddPasteboardChangedObserver(
  ...
  return ctocpp_->AddPasteboardChangedObserver(
      new ArkPasteBoardObserverAdapterImpl(callback));
}

void ArkPasteBoardClientAdapterWrapper::RemovePasteboardChangedObserver(
    int32_t callbackId) {
  if (!ctocpp_) {
    return;
  }
