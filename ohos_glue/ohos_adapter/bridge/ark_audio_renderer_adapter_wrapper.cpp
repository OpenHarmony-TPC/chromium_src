...
ArkAudioRendererAdapterWrapper::ArkAudioRendererAdapterWrapper(
    ArkWebRefPtr<ArkAudioRendererAdapter> ref)
    : ctocpp_(ref) {
}

int32_t ArkAudioRendererAdapterWrapper::Create(
    const std::shared_ptr<NWeb::AudioRendererOptionsAdapter> options,
  ...
  if (CHECK_SHARED_PTR_IS_NULL(options)) {
    ...
    result =
        ctocpp_->Create(new ArkAudioRendererOptionsAdapterImpl(options), str);
  }
...
int32_t ArkAudioRendererAdapterWrapper::Write(uint8_t *buffer,
                                              size_t bufferSize) {
...
int32_t ArkAudioRendererAdapterWrapper::SetAudioRendererCallback(
  ...
  return ctocpp_->SetAudioRendererCallback(
      new ArkAudioRendererCallbackAdapterImpl(callback));
}
