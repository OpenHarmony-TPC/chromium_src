...
ArkSystemPropertiesAdapterWrapper::ArkSystemPropertiesAdapterWrapper(
    ArkWebRefPtr<ArkSystemPropertiesAdapter> ref)
    : ctocpp_(ref) {
}
...
OHOS::NWeb::ProductDeviceType
ArkSystemPropertiesAdapterWrapper::GetProductDeviceType() {
  if (!ctocpp_) {
...
bool ArkSystemPropertiesAdapterWrapper::GetBoolParameter(const std::string &key,
                                                         bool defaultValue) {
...
std::vector<FrameRateSetting> ArkSystemPropertiesAdapterWrapper::GetLTPOConfig(
    const std::string &settingName) {
  if (!ctocpp_) {
    return {};
  }
  ArkWebString ark_setting_name = ArkWebStringClassToStruct(settingName);
  ArkFrameRateSettingAdapterVector ark_vector =
      ctocpp_->GetLTPOConfig(ark_setting_name);
  std::vector<FrameRateSetting> result{};

  for (int i = 0; i < ark_vector.size; i++) {
    result.emplace_back(
        FrameRateSetting{ark_vector.value[i].min_, ark_vector.value[i].max_,
                         ark_vector.value[i].preferredFrameRate_});
  }
  ArkWebStringStructRelease(ark_setting_name);
  SAFE_FREE(ark_vector.value, ark_vector.ark_web_mem_free_func);
  // ArkWebBasicVectorStructRelease(ark_vector);
