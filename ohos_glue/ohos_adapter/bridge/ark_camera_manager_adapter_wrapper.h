...
public:
  ...
  int32_t Create(std::shared_ptr<NWeb::CameraStatusCallbackAdapter>
                     cameraStatusCallback) override;

  std::vector<std::shared_ptr<NWeb::VideoDeviceDescriptorAdapter>>
  GetDevicesInfo() override;

  int32_t ReleaseCameraManger() override;

  int32_t GetExposureModes(
      std::vector<NWeb::ExposureModeAdapter> &exposureModesAdapter) override;

  int32_t GetCurrentExposureMode(
      NWeb::ExposureModeAdapter &exposureModeAdapter) override;

  std::shared_ptr<NWeb::VideoCaptureRangeAdapter>
  GetCaptionRangeById(NWeb::RangeIDAdapter rangeId) override;
