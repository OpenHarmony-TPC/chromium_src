...
class ArkAafwkAppMgrClientAdapterWrapper
...
public:
  ...
  void AttachRenderProcess(
      std::shared_ptr<NWeb::AafwkRenderSchedulerHostAdapter> adapter) override;

  int GetRenderProcessTerminationStatus(pid_t renderPid, int &status) override;

  int StartChildProcess(const std::string& renderParam,
                         int32_t ipcFd,
                         int32_t sharedFd,
                         int32_t crashFd,
                         pid_t& renderPid,
                         const std::string& processType) override;
