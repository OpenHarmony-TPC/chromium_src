...
ArkIMFTextListenerAdapterImpl::ArkIMFTextListenerAdapterImpl(
    std::shared_ptr<OHOS::NWeb::IMFTextListenerAdapter> ref)
    : real_(ref) {
}
...
void ArkIMFTextListenerAdapterImpl::SendKeyboardStatus(
    const int32_t &keyboardStatus) {
  real_->SendKeyboardStatus(
      (OHOS::NWeb::IMFAdapterKeyboardStatus)keyboardStatus);
}

void ArkIMFTextListenerAdapterImpl::SendFunctionKey(
    ArkWebRefPtr<ArkIMFAdapterFunctionKeyAdapter> functionKey) {
  if (CHECK_REF_PTR_IS_NULL(functionKey)) {
    real_->SendFunctionKey(nullptr);
  }

  real_->SendFunctionKey(
      std::make_shared<ArkIMFAdapterFunctionKeyAdapterWrapper>(functionKey));
}
...
void ArkIMFTextListenerAdapterImpl::HandleSetSelection(int32_t start,
                                                       int32_t end) {
...
void ArkIMFTextListenerAdapterImpl::HandleSelect(int32_t keyCode,
...
ArkWebU16String
ArkIMFTextListenerAdapterImpl::GetLeftTextOfCursor(int32_t number) {
...
ArkWebU16String
ArkIMFTextListenerAdapterImpl::GetRightTextOfCursor(int32_t number) {
...
int32_t
ArkIMFTextListenerAdapterImpl::SetPreviewText(const ArkWebU16String &text,
