...
ArkAudioSystemManagerAdapterWrapper::ArkAudioSystemManagerAdapterWrapper(
    ArkWebRefPtr<ArkAudioSystemManagerAdapter> ref)
    : ctocpp_(ref) {
}
...
int32_t ArkAudioSystemManagerAdapterWrapper::RequestAudioFocus(
  ...
  return ctocpp_->RequestAudioFocus(
      new ArkAudioInterruptAdapterImpl(audioInterrupt));
}
...
int32_t ArkAudioSystemManagerAdapterWrapper::AbandonAudioFocus(
  ...
  return ctocpp_->AbandonAudioFocus(
      new ArkAudioInterruptAdapterImpl(audioInterrupt));
}
...
int32_t ArkAudioSystemManagerAdapterWrapper::SetAudioManagerInterruptCallback(
  ...
  return ctocpp_->SetAudioManagerInterruptCallback(
      new ArkAudioManagerCallbackAdapterImpl(callback));
}

int32_t
ArkAudioSystemManagerAdapterWrapper::UnsetAudioManagerInterruptCallback() {
  if (!ctocpp_) {
    return -1;
  }
...
std::vector<std::shared_ptr<NWeb::AudioDeviceDescAdapter>>
ArkAudioSystemManagerAdapterWrapper::GetDevices(NWeb::AdapterDeviceFlag flag) {
  if (!ctocpp_) {
    return std::vector<std::shared_ptr<NWeb::AudioDeviceDescAdapter>>();
  }

  ArkAudioDeviceDescAdapterVector ark_vector =
      ctocpp_->GetDevices((int32_t)flag);
...
int32_t
ArkAudioSystemManagerAdapterWrapper::SelectAudioDeviceById(int32_t deviceId,
                                                           bool isInput) {
  if (!ctocpp_) {
    return -1;
  }
...
int32_t ArkAudioSystemManagerAdapterWrapper::SetDeviceChangeCallback(
    const std::shared_ptr<OHOS::NWeb::AudioManagerDeviceChangeCallbackAdapter>
        callback) {
  if (!ctocpp_) {
    return -1;
  }
  ...
  return ctocpp_->SetDeviceChangeCallback(
      new ArkAudioManagerDeviceChangeCallbackAdapterImpl(callback));
}
...
ArkAudioSystemManagerAdapterWrapper::GetDefaultOutputDevice() {
  if (!ctocpp_) {
    return nullptr;
  }

  ArkWebRefPtr<ArkAudioDeviceDescAdapter> adapter =
      ctocpp_->GetDefaultOutputDevice();
...
std::shared_ptr<NWeb::AudioDeviceDescAdapter>
ArkAudioSystemManagerAdapterWrapper::GetDefaultInputDevice() {
  if (!ctocpp_) {
    return nullptr;
  }

  ArkWebRefPtr<ArkAudioDeviceDescAdapter> adapter =
      ctocpp_->GetDefaultInputDevice();
  if (CHECK_REF_PTR_IS_NULL(adapter)) {
    return nullptr;
  }
