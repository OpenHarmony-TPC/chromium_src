...
ArkNativeImageAdapterWrapper::ArkNativeImageAdapterWrapper(
    ArkWebRefPtr<ArkNativeImageAdapter> ref)
    : ctocpp_(ref) {
}

void ArkNativeImageAdapterWrapper::CreateNativeImage(uint32_t textureId,
                                                     uint32_t textureTarget) {
  return ctocpp_->CreateNativeImage(textureId, textureTarget);
}

NWeb::NWebNativeWindow
ArkNativeImageAdapterWrapper::AquireNativeWindowFromNativeImage() {
...
int32_t ArkNativeImageAdapterWrapper::SetOnFrameAvailableListener(
  ...
  return ctocpp_->SetOnFrameAvailableListener(
      new ArkFrameAvailableListenerImpl(listener));
}
