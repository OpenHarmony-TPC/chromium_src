...
ArkIMFAdapterWrapper::ArkIMFAdapterWrapper(ArkWebRefPtr<ArkIMFAdapter> ref)
    : ctocpp_(ref) {
}

bool ArkIMFAdapterWrapper::Attach(
    std::shared_ptr<OHOS::NWeb::IMFTextListenerAdapter> listener,
    bool isShowKeyboard) {
  if (CHECK_SHARED_PTR_IS_NULL(listener)) {
    return ctocpp_->Attach(nullptr, isShowKeyboard);
  }

  return ctocpp_->Attach(new ArkIMFTextListenerAdapterImpl(listener),
                         isShowKeyboard);
}
...
bool ArkIMFAdapterWrapper::Attach(
  ...
  if (!CHECK_SHARED_PTR_IS_NULL(listener) &&
      !CHECK_SHARED_PTR_IS_NULL(config)) {
    return ctocpp_->Attach(new ArkIMFTextListenerAdapterImpl(listener),
                           isShowKeyboard,
                           new ArkIMFTextConfigAdapterImpl(config));
    ...
    return ctocpp_->Attach(new ArkIMFTextListenerAdapterImpl(listener),
                           isShowKeyboard, nullptr);
  }
}

void ArkIMFAdapterWrapper::ShowCurrentInput(
    const OHOS::NWeb::IMFAdapterTextInputType &inputType) {
...
void ArkIMFAdapterWrapper::OnCursorUpdate(
    const std::shared_ptr<OHOS::NWeb::IMFCursorInfoAdapter> cursorInfo) {
...
void ArkIMFAdapterWrapper::OnSelectionChange(std::u16string text, int start,
                                             int end) {
