...
ArkPlayerAdapterWrapper::ArkPlayerAdapterWrapper(
    ArkWebRefPtr<ArkPlayerAdapter> ref)
    : ctocpp_(ref) {
}

int32_t ArkPlayerAdapterWrapper::SetPlayerCallback(
    std::shared_ptr<OHOS::NWeb::PlayerCallbackAdapter> callbackAdapter) {
  if (CHECK_SHARED_PTR_IS_NULL(callbackAdapter)) {
    return ctocpp_->SetPlayerCallback(nullptr);
  }

  return ctocpp_->SetPlayerCallback(
      new ArkPlayerCallbackAdapterImpl(callbackAdapter));
}
...
int32_t ArkPlayerAdapterWrapper::SetSource(int32_t fd, int64_t offset,
                                           int64_t size) {
  return ctocpp_->SetSource(fd, offset, size);
}

int32_t ArkPlayerAdapterWrapper::SetVideoSurface(
    std::shared_ptr<OHOS::NWeb::IConsumerSurfaceAdapter> cSurfaceAdapter) {
  std::shared_ptr<ArkIConsumerSurfaceAdapterWrapper> wrapper =
      std::static_pointer_cast<ArkIConsumerSurfaceAdapterWrapper>(
          cSurfaceAdapter);
  return ctocpp_->SetVideoSurface(wrapper->ctocpp_);
}

int32_t ArkPlayerAdapterWrapper::SetVolume(float leftVolume,
                                           float rightVolume) {
  return ctocpp_->SetVolume(leftVolume, rightVolume);
}

int32_t ArkPlayerAdapterWrapper::Seek(int32_t mSeconds,
                                      OHOS::NWeb::PlayerSeekMode mode) {
...
int32_t
ArkPlayerAdapterWrapper::SetPlaybackSpeed(OHOS::NWeb::PlaybackRateMode mode) {
