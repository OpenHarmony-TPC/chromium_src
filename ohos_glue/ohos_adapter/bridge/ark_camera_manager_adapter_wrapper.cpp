...
ArkCameraManagerAdapterWrapper::ArkCameraManagerAdapterWrapper(
    ArkWebRefPtr<ArkCameraManagerAdapter> ref)
    : ctocpp_(ref) {
}

int32_t ArkCameraManagerAdapterWrapper::Create(
    std::shared_ptr<NWeb::CameraStatusCallbackAdapter> cameraStatusCallback) {
  if (!ctocpp_) {
    return -1;
  }
  ...
  return ctocpp_->Create(
      new ArkCameraStatusCallbackAdapterImpl(cameraStatusCallback));
}

std::vector<std::shared_ptr<NWeb::VideoDeviceDescriptorAdapter>>
ArkCameraManagerAdapterWrapper::GetDevicesInfo() {
  if (!ctocpp_) {
...
int32_t ArkCameraManagerAdapterWrapper::GetExposureModes(
    std::vector<NWeb::ExposureModeAdapter> &exposureModesAdapter) {
  if (!ctocpp_) {
    return -1;
  }
...
int32_t ArkCameraManagerAdapterWrapper::GetCurrentExposureMode(
    NWeb::ExposureModeAdapter &exposureModeAdapter) {
  if (!ctocpp_) {
    return -1;
  }
  int32_t temp;
...
std::shared_ptr<NWeb::VideoCaptureRangeAdapter>
ArkCameraManagerAdapterWrapper::GetCaptionRangeById(
    NWeb::RangeIDAdapter rangeId) {
  if (!ctocpp_) {
    return nullptr;
  }

  ArkWebRefPtr<ArkVideoCaptureRangeAdapter> adapter =
      ctocpp_->GetCaptionRangeById((int32_t)rangeId);
...
bool ArkCameraManagerAdapterWrapper::IsFocusModeSupported(
    NWeb::FocusModeAdapter focusMode) {
  if (!ctocpp_) {
    return false;
  }
...
bool ArkCameraManagerAdapterWrapper::IsFlashModeSupported(
    NWeb::FlashModeAdapter flashMode) {
  if (!ctocpp_) {
    return false;
  }
...
int32_t
ArkCameraManagerAdapterWrapper::StopSession(NWeb::CameraStopType stopType) {
  if (!ctocpp_) {
    return -1;
  }
...
int32_t ArkCameraManagerAdapterWrapper::StartStream(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(listener) &&
      CHECK_SHARED_PTR_IS_NULL(captureParams)) {
    result = ctocpp_->StartStream(str, nullptr, nullptr);
  } else if (!CHECK_SHARED_PTR_IS_NULL(listener) &&
             !CHECK_SHARED_PTR_IS_NULL(captureParams)) {
    result = ctocpp_->StartStream(
        str, new ArkVideoCaptureParamsAdapterImpl(captureParams),
        new ArkCameraBufferListenerAdapterImpl(listener));
  } else if (CHECK_SHARED_PTR_IS_NULL(listener)) {
    result = ctocpp_->StartStream(
        str, new ArkVideoCaptureParamsAdapterImpl(captureParams), nullptr);
  } else {
    result = ctocpp_->StartStream(
        str, nullptr, new ArkCameraBufferListenerAdapterImpl(listener));
  }
...
void ArkCameraManagerAdapterWrapper::SetCameraStatus(
    NWeb::CameraStatusAdapter status) {
  if (!ctocpp_) {
    return;
  }
