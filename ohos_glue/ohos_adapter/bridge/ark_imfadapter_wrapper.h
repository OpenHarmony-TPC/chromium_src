...
public:
  ArkIMFAdapterWrapper(ArkWebRefPtr<ArkIMFAdapter>);

  bool Attach(std::shared_ptr<OHOS::NWeb::IMFTextListenerAdapter> listener,
  ...
  void ShowCurrentInput(
      const OHOS::NWeb::IMFAdapterTextInputType &inputType) override;

  void HideTextInput() override;

  void Close() override;

  void OnCursorUpdate(const std::shared_ptr<OHOS::NWeb::IMFCursorInfoAdapter>
                          cursorInfo) override;
