...
class ArkMediaCodecDecoderAdapterWrapper
    : public OHOS::NWeb::MediaCodecDecoderAdapter {
public:
  ArkMediaCodecDecoderAdapterWrapper(ArkWebRefPtr<ArkMediaCodecDecoderAdapter>);

  OHOS::NWeb::DecoderAdapterCode
  CreateVideoDecoderByMime(const std::string &mimetype) override;

  OHOS::NWeb::DecoderAdapterCode
  CreateVideoDecoderByName(const std::string &name) override;
  ...
  OHOS::NWeb::DecoderAdapterCode
  QueueInputBufferDec(uint32_t index, int64_t presentationTimeUs, int32_t size,
                      int32_t offset, OHOS::NWeb::BufferFlag flag) override;

  OHOS::NWeb::DecoderAdapterCode GetOutputFormatDec(
      std::shared_ptr<OHOS::NWeb::DecoderFormatAdapter> format) override;

  OHOS::NWeb::DecoderAdapterCode ReleaseOutputBufferDec(uint32_t index,
