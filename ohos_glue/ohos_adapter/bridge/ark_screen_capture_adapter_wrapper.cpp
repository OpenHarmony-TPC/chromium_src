...
ArkScreenCaptureAdapterWrapper::ArkScreenCaptureAdapterWrapper(
    ArkWebRefPtr<ArkScreenCaptureAdapter> ref)
    : ctocpp_(ref) {
}

int32_t ArkScreenCaptureAdapterWrapper::Init(
    const std::shared_ptr<NWeb::ScreenCaptureConfigAdapter> config) {
...
int32_t ArkScreenCaptureAdapterWrapper::SetCaptureCallback(
  ...
  return ctocpp_->SetCaptureCallback(
      new ArkScreenCaptureCallbackAdapterImpl(callback));
}

std::shared_ptr<OHOS::NWeb::SurfaceBufferAdapter>
ArkScreenCaptureAdapterWrapper::AcquireVideoBuffer() {
