...
void ArkNetProxyAdapterCToCpp::RegNetProxyEvent(
  ...
  ARK_WEB_CTOCPP_CHECK_FUNC_MEMBER(_struct, reg_net_proxy_event, );
...
ARK_WEB_NO_SANITIZE
void ArkNetProxyAdapterCToCpp::GetProperty(ArkWebString &host, uint16_t &port,
                                           ArkWebString &pacUrl,
  ...
  ARK_WEB_CTOCPP_CHECK_FUNC_MEMBER(_struct, get_property, );

  // Execute
  _struct->get_property(_struct, &host, &port, &pacUrl, &exclusion);
}
...
template <>
ArkWebBridgeType
    ArkWebCToCppRefCounted<ArkNetProxyAdapterCToCpp, ArkNetProxyAdapter,
