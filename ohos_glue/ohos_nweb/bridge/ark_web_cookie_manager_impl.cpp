...
ArkWebCookieManagerImpl::ArkWebCookieManagerImpl(
    std::shared_ptr<OHOS::NWeb::NWebCookieManager> nweb_cookie_manager)
...
void ArkWebCookieManagerImpl::Store(
    ArkWebRefPtr<ArkWebBoolValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
    nweb_cookie_manager_->Store(nullptr);
    return;
  }

  nweb_cookie_manager_->Store(
      std::make_shared<ArkWebBoolValueCallbackWrapper>(callback));
}

int ArkWebCookieManagerImpl::SetCookie(const ArkWebString &url,
                                       const ArkWebString &value,
                                       bool incognito_mode) {
  return nweb_cookie_manager_->SetCookie(ArkWebStringStructToClass(url),
                                         ArkWebStringStructToClass(value),
                                         incognito_mode);
}

void ArkWebCookieManagerImpl::SetCookie(
    const ArkWebString &url, const ArkWebString &value,
    ArkWebRefPtr<ArkWebBoolValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
    nweb_cookie_manager_->SetCookie(ArkWebStringStructToClass(url),
                                    ArkWebStringStructToClass(value), nullptr);
    return;
  }

  nweb_cookie_manager_->SetCookie(
      ArkWebStringStructToClass(url), ArkWebStringStructToClass(value),
      std::make_shared<ArkWebBoolValueCallbackWrapper>(callback));
}
...
void ArkWebCookieManagerImpl::ExistCookies(
    ArkWebRefPtr<ArkWebBoolValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
  ...
  nweb_cookie_manager_->ExistCookies(
      std::make_shared<ArkWebBoolValueCallbackWrapper>(callback));
}

ArkWebString ArkWebCookieManagerImpl::ReturnCookie(const ArkWebString &url,
                                                   bool &is_valid,
                                                   bool incognito_mode) {
  return ArkWebStringClassToStruct(nweb_cookie_manager_->ReturnCookie(
      ArkWebStringStructToClass(url), is_valid, incognito_mode));
}

void ArkWebCookieManagerImpl::ReturnCookie(
    const ArkWebString &url, ArkWebRefPtr<ArkWebStringValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
...
void ArkWebCookieManagerImpl::ConfigCookie(
  ...
  if (CHECK_REF_PTR_IS_NULL(callback)) {
    nweb_cookie_manager_->ConfigCookie(ArkWebStringStructToClass(url),
                                       ArkWebStringStructToClass(value),
                                       nullptr);
    return;
  }

  nweb_cookie_manager_->ConfigCookie(
      ArkWebStringStructToClass(url), ArkWebStringStructToClass(value),
      std::make_shared<ArkWebLongValueCallbackWrapper>(callback));
}

void ArkWebCookieManagerImpl::DeleteSessionCookies(
    ArkWebRefPtr<ArkWebBoolValueCallback> callback) {
  if (CHECK_REF_PTR_IS_NULL(callback)) {
  ...
  nweb_cookie_manager_->DeleteSessionCookies(
      std::make_shared<ArkWebBoolValueCallbackWrapper>(callback));
}

void ArkWebCookieManagerImpl::DeleteCookieEntirely(
