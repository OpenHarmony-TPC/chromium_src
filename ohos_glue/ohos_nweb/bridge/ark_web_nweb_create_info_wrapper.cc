...
ArkWebNWebCreateInfoWrapper::ArkWebNWebCreateInfoWrapper(
    ArkWebRefPtr<ArkWebNWebCreateInfo> ark_web_nweb_create_info)
...
ArkWebNWebCreateInfoWrapper::GetEngineInitArgs() {
  ArkWebRefPtr<ArkWebEngineInitArgs> ark_web_engine_init_args =
      ark_web_nweb_create_info_->GetEngineInitArgs();
  if (CHECK_REF_PTR_IS_NULL(ark_web_engine_init_args)) {
    return nullptr;
  }

  return std::make_shared<ArkWebEngineInitArgsWrapper>(
      ark_web_engine_init_args);
}

std::shared_ptr<OHOS::NWeb::NWebOutputFrameCallback>
ArkWebNWebCreateInfoWrapper::GetOutputFrameCallback() {
  ...
  return std::make_shared<ArkWebOutputFrameCallbackWrapper>(
