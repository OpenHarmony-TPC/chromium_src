...
ArkWebEngineImpl::ArkWebEngineImpl(
    ...
    : nweb_engine_(nweb_engine) {
}

ArkWebRefPtr<ArkWebEngine> ArkWebEngine::GetInstance() {
  std::shared_ptr<OHOS::NWeb::NWebEngine> nweb_engine =
      OHOS::NWeb::NWebEngine::GetInstance();
...
ArkWebEngineImpl::CreateNWeb(ArkWebRefPtr<ArkWebNWebCreateInfo> create_info) {
  std::shared_ptr<OHOS::NWeb::NWebCreateInfo> nweb_create_info = nullptr;
  if (!CHECK_REF_PTR_IS_NULL(create_info)) {
    nweb_create_info =
        std::make_shared<ArkWebNWebCreateInfoWrapper>(create_info);
  }

  std::shared_ptr<OHOS::NWeb::NWeb> nweb =
...
ArkWebRefPtr<ArkWebDownloadManager> ArkWebEngineImpl::GetDownloadManager() {
  std::shared_ptr<OHOS::NWeb::NWebDownloadManager> nweb_download_manager =
      nweb_engine_->GetDownloadManager();
...
void ArkWebEngineImpl::InitializeWebEngine(
...
void ArkWebEngineImpl::PrepareForPageLoad(const ArkWebString &url,
                                          bool preconnectable,
                                          int32_t num_sockets) {
  nweb_engine_->PrepareForPageLoad(ArkWebStringStructToClass(url),
                                   preconnectable, num_sockets);
}
...
void ArkWebEngineImpl::SetRenderProcessMode(int32_t mode) {
  nweb_engine_->SetRenderProcessMode(
      static_cast<ArkWebRenderProcessMode>(mode));
}
...
void ArkWebEngineImpl::ClearPrefetchedResource(
    const ArkWebStringVector &cache_key_list) {
  nweb_engine_->ClearPrefetchedResource(
      ArkWebStringVectorStructToClass(cache_key_list));
}
...
void ArkWebEngineImpl::SetHostIP(const ArkWebString &hostName,
                                 ...
                                 int32_t aliveTime) {
  nweb_engine_->SetHostIP(ArkWebStringStructToClass(hostName),
