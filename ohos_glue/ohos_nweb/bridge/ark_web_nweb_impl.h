...
public:
  ...
  void OnTouchMove(const ArkWebTouchPointInfoVector &touch_point_infos,
                   bool from_overlay) override;
  ...
  void SendMouseWheelEvent(double x, double y, double delta_x,
   ...
   */
  void ExecuteJavaScript(const ArkWebString &code,
  ...
  void PutDownloadCallback(
      ArkWebRefPtr<ArkWebDownloadCallback> download_listener) override;
   ...
   */
  void
  PutAccessibilityEventCallback(ArkWebRefPtr<ArkWebAccessibilityEventCallback>
  ...
  void PutAccessibilityIdGenerator(
      AccessibilityIdGenerateFunc accessibility_id_generator) override;
   ...
   */
  int Load(const ArkWebString &url,
           const ArkWebStringMap &additional_http_headers) override;
   ...
   *        may be null.
   */
  int LoadWithDataAndBaseUrl(const ArkWebString &base_url,
                             const ArkWebString &data,
                             const ArkWebString &mime_type,
                             const ArkWebString &encoding,
                             const ArkWebString &history_url) override;
   ...
   * @param encoding: the encoding of the data This value may be null.
   */
  int LoadWithData(const ArkWebString &data, const ArkWebString &mime_type,
                   const ArkWebString &encoding) override;
   ...
   */
  void UnregisterArkJSfunction(const ArkWebString &object_name,
                               const ArkWebStringVector &method_list) override;
  ...
  void SetNWebJavaScriptResultCallBack(
      ArkWebRefPtr<ArkWebJsResultCallback> callback) override;
   ...
   */
  void
  StoreWebArchive(const ArkWebString &base_name, bool auto_name,
   ...
   */
  void PostWebMessage(const ArkWebString &message,
                      const ArkWebStringVector &ports,
                      const ArkWebString &target_uri) override;
   ...
   */
  void PostPortMessage(const ArkWebString &port_handle,
                       const ArkWebMessage &data) override;
   ...
   */
  void UpdateLocale(const ArkWebString &language,
                    const ArkWebString &region) override;
  ...
  bool GetFavicon(const void **data, size_t &width, size_t &height,
                  int &color_type, int &alpha_type) override;
   ...
   */
  void PutReleaseSurfaceCallback(ArkWebRefPtr<ArkWebReleaseSurfaceCallback>
                                     release_surface_listener) override;
   ...
   */
  bool GetCertChainDerData(ArkWebStringVector &cert_chain_data,
                           bool is_single_cert) override;
  ...
  void RegisterScreenLockFunction(
      int32_t window_id,
      ArkWebRefPtr<ArkWebScreenLockCallback> callback) override;
   ...
   */
  void PrefetchPage(const ArkWebString &url,
                    const ArkWebStringMap &additional_http_headers) override;
   ...
   * @brief Set draw rect.
   */
  void SetDrawRect(int32_t x, int32_t y, int32_t width,
   ...
   * @return title string for the current page.
   */
  int PostUrl(const ArkWebString &url,
              const ArkWebCharVector &post_data) override;

  /**
   * @brief Inject the JavaScript before WebView load the DOM tree.
   */
  void
  JavaScriptOnDocumentStart(const ArkWebStringVectorMap &script_items) override;
  ...
  GetFocusedAccessibilityNodeInfo(int64_t accessibility_id,
                                  bool is_accessibility_focus) override;
   ...
   */
  ArkWebRefPtr<ArkWebAccessibilityNodeInfo>
  GetAccessibilityNodeInfoById(int64_t accessibility_id) override;
   ...
   */
  void CallH5Function(int32_t routingId, int32_t h5ObjectId,
  ...
  void RegisterNativeArkJSFunction(
      const char *objName,
      const ArkWebJsProxyCallbackVector &callbacks) override;
  ...
  void RegisterNativeValideCallback(
      const char *webName, const NativeArkWebOnValidCallback callback) override;
   ...
   */
  void
  JavaScriptOnDocumentEnd(const ArkWebStringVectorMap &script_items) override;
