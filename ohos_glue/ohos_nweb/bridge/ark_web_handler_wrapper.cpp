...
ArkWebHandlerWrapper::ArkWebHandlerWrapper(
    ArkWebRefPtr<ArkWebHandler> ark_web_handler)
...
void ArkWebHandlerWrapper::OnPageIcon(const void *data, size_t width,
                                      size_t height,
                                      ArkWebImageColorType color_type,
                                      ArkWebImageAlphaType alpha_type) {
  ark_web_handler_->OnPageIcon(data, width, height,
                               static_cast<int>(color_type),
                               static_cast<int>(alpha_type));
}
...
void ArkWebHandlerWrapper::OnHttpError(
...
bool ArkWebHandlerWrapper::OnConsoleLog(
    std::shared_ptr<OHOS::NWeb::NWebConsoleLog> console_log) {
...
void ArkWebHandlerWrapper::OnPageLoadEnd(int http_status_code,
...
void ArkWebHandlerWrapper::OnPageLoadError(int error_code,
...
void ArkWebHandlerWrapper::OnDesktopIconUrl(const std::string &icon_url,
                                            bool precomposed) {
...
void ArkWebHandlerWrapper::OnGeolocationShow(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(callback)) {
    ...
    ark_web_handler_->OnGeolocationShow(
        stOrigin, new ArkWebGeoLocationCallbackImpl(callback));
  }
...
bool ArkWebHandlerWrapper::OnFileSelectorShow(
...
void ArkWebHandlerWrapper::OnResourceLoadError(
    std::shared_ptr<OHOS::NWeb::NWebUrlResourceRequest> request,
...
void ArkWebHandlerWrapper::OnPermissionRequest(
...
void ArkWebHandlerWrapper::OnTouchSelectionChanged(
    std::shared_ptr<OHOS::NWeb::NWebTouchHandleState> insert_handle,
  ...
  if (!CHECK_SHARED_PTR_IS_NULL(start_selection_handle)) {
    ark_web_start_selection_handle =
        new ArkWebTouchHandleStateImpl(start_selection_handle);
  }

  ArkWebRefPtr<ArkWebTouchHandleState> ark_web_end_selection_handle = nullptr;
  if (!CHECK_SHARED_PTR_IS_NULL(end_selection_handle)) {
    ark_web_end_selection_handle =
        new ArkWebTouchHandleStateImpl(end_selection_handle);
  }

  ark_web_handler_->OnTouchSelectionChanged(ark_web_insert_handle,
                                            ark_web_start_selection_handle,
                                            ark_web_end_selection_handle);
}

bool ArkWebHandlerWrapper::OnHandleInterceptRequest(
    std::shared_ptr<OHOS::NWeb::NWebUrlResourceRequest> request,
    std::shared_ptr<OHOS::NWeb::NWebUrlResourceResponse> response) {
  ArkWebRefPtr<ArkWebUrlResourceRequest> ark_web_request = nullptr;
  if (!CHECK_SHARED_PTR_IS_NULL(request)) {
  ...
  return ark_web_handler_->OnHandleInterceptRequest(ark_web_request,
                                                    ark_web_response);
}

void ArkWebHandlerWrapper::OnRefreshAccessedHistory(const std::string &url,
                                                    bool is_reload) {
  ArkWebString stUrl = ArkWebStringClassToStruct(url);
...
void ArkWebHandlerWrapper::OnPermissionRequestCanceled(
    std::shared_ptr<OHOS::NWeb::NWebAccessRequest> request) {
  if (CHECK_SHARED_PTR_IS_NULL(request)) {
  ...
  ark_web_handler_->OnPermissionRequestCanceled(
      new ArkWebAccessRequestImpl(request));
}

bool ArkWebHandlerWrapper::OnHandleInterceptUrlLoading(
    std::shared_ptr<OHOS::NWeb::NWebUrlResourceRequest> request) {
  if (CHECK_SHARED_PTR_IS_NULL(request)) {
    return ark_web_handler_->OnHandleInterceptUrlLoading(nullptr);
  }

  return ark_web_handler_->OnHandleInterceptUrlLoading(
      new ArkWebUrlResourceRequestImpl(request));
}

bool ArkWebHandlerWrapper::RunQuickMenu(
...
bool ArkWebHandlerWrapper::RunContextMenu(
    std::shared_ptr<OHOS::NWeb::NWebContextMenuParams> params,
...
bool ArkWebHandlerWrapper::FilterScrollEvent(const float x, const float y,
...
std::vector<std::string> ArkWebHandlerWrapper::VisitedUrlHistory() {
  ...
  std::vector<std::string> objUrlHistory =
      ArkWebStringVectorStructToClass(stUrlHistory);
  ArkWebStringVectorStructRelease(stUrlHistory);
  return objUrlHistory;
}

void ArkWebHandlerWrapper::OnWindowNewByJS(
    const std::string &target_url, bool is_alert, bool is_user_trigger,
  ...
  if (CHECK_SHARED_PTR_IS_NULL(handler)) {
    ark_web_handler_->OnWindowNewByJS(stTargetUrl, is_alert, is_user_trigger,
                                      nullptr);
  } else {
    ark_web_handler_->OnWindowNewByJS(stTargetUrl, is_alert, is_user_trigger,
                                      new ArkWebControllerHandlerImpl(handler));
  }
...
bool ArkWebHandlerWrapper::OnAlertDialogByJS(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    ...
    flag = ark_web_handler_->OnAlertDialogByJS(
        stUrl, stMessage, new ArkWebJsDialogResultImpl(result));
  }
...
bool ArkWebHandlerWrapper::OnBeforeUnloadByJS(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    ...
    flag = ark_web_handler_->OnBeforeUnloadByJS(
        stUrl, stMessage, new ArkWebJsDialogResultImpl(result));
  }
...
bool ArkWebHandlerWrapper::OnPromptDialogByJS(
    const std::string &url, const std::string &message,
  ...
  bool flag = false;
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    flag = ark_web_handler_->OnPromptDialogByJS(stUrl, stMessage,
                                                stDefaultValue, nullptr);
  } else {
...
bool ArkWebHandlerWrapper::OnConfirmDialogByJS(
  ...
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    ...
    flag = ark_web_handler_->OnConfirmDialogByJS(
        stUrl, stMessage, new ArkWebJsDialogResultImpl(result));
  }
...
bool ArkWebHandlerWrapper::OnHttpAuthRequestByJS(
    std::shared_ptr<OHOS::NWeb::NWebJSHttpAuthResult> result,
  ...
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    ...
    flag = ark_web_handler_->OnHttpAuthRequestByJS(
        new ArkWebJsHttpAuthResultImpl(result), stHost, stRealm);
  }
...
bool ArkWebHandlerWrapper::OnSslErrorRequestByJS(
    std::shared_ptr<OHOS::NWeb::NWebJSSslErrorResult> result,
    ArkWebSslError error) {
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    return ark_web_handler_->OnSslErrorRequestByJS(nullptr,
                                                   static_cast<int>(error));
  }

  return ark_web_handler_->OnSslErrorRequestByJS(
      new ArkWebJsSslErrorResultImpl(result), static_cast<int>(error));
}

bool ArkWebHandlerWrapper::OnSslSelectCertRequestByJS(
  ...
  bool flag = false;
  if (CHECK_SHARED_PTR_IS_NULL(result)) {
    flag = ark_web_handler_->OnSslSelectCertRequestByJS(nullptr, stHost, port,
                                                        stKeyTypes, stIssuers);
  } else {
    flag = ark_web_handler_->OnSslSelectCertRequestByJS(
        new ArkWebJsSslSelectCertResultImpl(result), stHost, port, stKeyTypes,
        stIssuers);
  }
...
bool ArkWebHandlerWrapper::OnPreKeyEvent(
...
void ArkWebHandlerWrapper::OnScaleChanged(float old_scale_factor,
...
bool ArkWebHandlerWrapper::OnCursorChange(
    const ArkWebCursorType &type,
    std::shared_ptr<OHOS::NWeb::NWebCursorInfo> info) {
  if (CHECK_SHARED_PTR_IS_NULL(info)) {
    return ark_web_handler_->OnCursorChange(static_cast<int32_t>(type),
                                            nullptr);
  }

  return ark_web_handler_->OnCursorChange(static_cast<int32_t>(type),
                                          new ArkWebCursorInfoImpl(info));
}
...
void ArkWebHandlerWrapper::OnFullScreenEnter(
  ...
  ark_web_handler_->OnFullScreenEnter(
      new ArkWebFullScreenExitHandlerImpl(handler));
}
...
void ArkWebHandlerWrapper::OnDataResubmission(
    std::shared_ptr<OHOS::NWeb::NWebDataResubmissionCallback> handler) {
  if (CHECK_SHARED_PTR_IS_NULL(handler)) {
    ark_web_handler_->OnDataResubmission(nullptr);
    return;
  }

  ark_web_handler_->OnDataResubmission(
      new ArkWebDataResubmissionCallbackImpl(handler));
}
...
bool ArkWebHandlerWrapper::OnUnProcessedKeyEvent(
    std::shared_ptr<OHOS::NWeb::NWebKeyEvent> event) {
  if (CHECK_SHARED_PTR_IS_NULL(event)) {
...
bool ArkWebHandlerWrapper::OnDragAndDropDataUdmf(
    std::shared_ptr<OHOS::NWeb::NWebDragData> drag_data) {
  if (CHECK_SHARED_PTR_IS_NULL(drag_data)) {
    return ark_web_handler_->OnDragAndDropDataUdmf(nullptr);
  }

  return ark_web_handler_->OnDragAndDropDataUdmf(
      new ArkWebDragDataImpl(drag_data));
}

void ArkWebHandlerWrapper::OnFirstContentfulPaint(
    int64_t navigation_start_tick, int64_t first_contentful_paint_ms) {
  ark_web_handler_->OnFirstContentfulPaint(navigation_start_tick,
...
void ArkWebHandlerWrapper::OnScreenCaptureRequest(
    std::shared_ptr<OHOS::NWeb::NWebScreenCaptureAccessRequest> request) {
  if (CHECK_SHARED_PTR_IS_NULL(request)) {
    ark_web_handler_->OnScreenCaptureRequest(nullptr);
    return;
  }

  ark_web_handler_->OnScreenCaptureRequest(
      new ArkWebScreenCaptureAccessRequestImpl(request));
}

void ArkWebHandlerWrapper::OnActivityStateChanged(int state,
                                                  ArkWebActivityType type) {
...
void ArkWebHandlerWrapper::OnOverScrollFlingVelocity(float x_velocity,
                                                     float y_velocity,
                                                     bool is_fling) {
  ark_web_handler_->OnOverScrollFlingVelocity(x_velocity, y_velocity, is_fling);
}

void ArkWebHandlerWrapper::OnNavigationEntryCommitted(
    std::shared_ptr<OHOS::NWeb::NWebLoadCommittedDetails> details) {
  if (CHECK_SHARED_PTR_IS_NULL(details)) {
    ark_web_handler_->OnNavigationEntryCommitted(nullptr);
    return;
  }

  ark_web_handler_->OnNavigationEntryCommitted(
      new ArkWebLoadCommittedDetailsImpl(details));
}

void ArkWebHandlerWrapper::OnNativeEmbedLifecycleChange(
    std::shared_ptr<OHOS::NWeb::NWebNativeEmbedDataInfo> dataInfo) {
  if (CHECK_SHARED_PTR_IS_NULL(dataInfo)) {
    ark_web_handler_->OnNativeEmbedLifecycleChange(nullptr);
    return;
  }

  ark_web_handler_->OnNativeEmbedLifecycleChange(
      new ArkWebNativeEmbedDataInfoImpl(dataInfo));
}

void ArkWebHandlerWrapper::OnNativeEmbedGestureEvent(
    std::shared_ptr<OHOS::NWeb::NWebNativeEmbedTouchEvent> touchEvent) {
  if (CHECK_SHARED_PTR_IS_NULL(touchEvent)) {
    ark_web_handler_->OnNativeEmbedGestureEvent(nullptr);
    return;
  }

  ark_web_handler_->OnNativeEmbedGestureEvent(
      new ArkWebNativeEmbedTouchEventImpl(touchEvent));
}
...
ArkWebHandlerWrapper::GetWordSelection(const std::string &text, int8_t offset) {
  ...
  ArkWebCharVector ark_result =
      ark_web_handler_->GetWordSelection(ark_text, offset);
...
void ArkWebHandlerWrapper::UpdateClippedSelectionBounds(int x, int y, int w,
                                                        int h) {
...
void ArkWebHandlerWrapper::CreateOverlay(void* data,
                                         size_t len,
                                         int width,
                                         int height,
                                         int offset_x,
                                         ...
                                         int point_x,
                                         int point_y) {
  ark_web_handler_->CreateOverlay(data,
                                  len,
                                  width,
                                  height,
                                  offset_x,
...
void ArkWebHandlerWrapper::OnOverlayStateChanged(int offset_x,
                                                 int offset_y,
                                                 int rect_width,
                                                 int rect_height) {
  ark_web_handler_->OnOverlayStateChanged(offset_x,
                                          offset_y,
                                          rect_width,
                                          rect_height);
}
