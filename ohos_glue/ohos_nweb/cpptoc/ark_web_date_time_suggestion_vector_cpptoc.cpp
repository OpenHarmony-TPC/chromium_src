...
ArkWebDateTimeSuggestionVector ArkWebDateTimeSuggestionVectorClassToStruct(
    const std::vector<std::shared_ptr<OHOS::NWeb::NWebDateTimeSuggestion>>
        &class_value) {
  ArkWebDateTimeSuggestionVector struct_value = {
  ...
  if (struct_value.size > 0) {
    ...
    for (auto it = class_value.begin(); it != class_value.end(); it++) {
      ArkWebRefPtr<ArkWebDateTimeSuggestion> ark_web_date_time_suggestion =
          new ArkWebDateTimeSuggestionImpl(*it);
      struct_value.value[count] =
          ArkWebDateTimeSuggestionCppToC::Invert(ark_web_date_time_suggestion);
      count++;
    }
  }
...
void ArkWebDateTimeSuggestionVectorStructRelease(
    ArkWebDateTimeSuggestionVector &struct_value) {
  struct_value.size = 0;
