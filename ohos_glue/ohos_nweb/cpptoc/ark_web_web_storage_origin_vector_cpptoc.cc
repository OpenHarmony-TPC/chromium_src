...
ArkWebWebStorageOriginVector ArkWebWebStorageOriginVectorClassToStruct(
    const std::vector<std::shared_ptr<OHOS::NWeb::NWebWebStorageOrigin>>
        &class_value) {
  ArkWebWebStorageOriginVector struct_value = {
      .size = class_value.size(), .ark_web_mem_free_func = ArkWebMemFree};
  if (struct_value.size > 0) {
    struct_value.value = (ark_web_web_storage_origin_t **)ArkWebMemMalloc(
        sizeof(ark_web_web_storage_origin_t *) * struct_value.size);

    int count = 0;
    for (auto it = class_value.begin(); it != class_value.end(); it++) {
      ArkWebRefPtr<ArkWebWebStorageOrigin> ark_web_web_storage_origin =
          new ArkWebWebStorageOriginImpl(*it);
      struct_value.value[count] =
          ArkWebWebStorageOriginCppToC::Invert(ark_web_web_storage_origin);
      count++;
    }
  }
...
void ArkWebWebStorageOriginVectorStructRelease(
    ArkWebWebStorageOriginVector &struct_value) {
  struct_value.size = 0;
