...
ArkWebMediaSourceInfoVector ArkWebMediaSourceInfoVectorClassToStruct(
    const std::vector<std::shared_ptr<OHOS::NWeb::NWebMediaSourceInfo>>
        &class_value) {
  ArkWebMediaSourceInfoVector struct_value = {
      .size = class_value.size(), .ark_web_mem_free_func = ArkWebMemFree};
  if (struct_value.size > 0) {
    struct_value.value = (ark_web_media_source_info_t **)ArkWebMemMalloc(
        sizeof(ark_web_media_source_info_t *) * struct_value.size);

    int count = 0;
    for (auto it = class_value.begin(); it != class_value.end(); it++) {
      ArkWebRefPtr<ArkWebMediaSourceInfo> ark_web_media_source_info =
          new ArkWebMediaSourceInfoImpl(*it);
      struct_value.value[count] =
          ArkWebMediaSourceInfoCppToC::Invert(ark_web_media_source_info);
      count++;
    }
  }
