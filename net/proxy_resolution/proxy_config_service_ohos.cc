...
bool GetProxyRules(const GetPropertyCallback& get_property,
  ...
  // There is one intentional  difference: by default Chromium uses the HTTP port (80)
  // for HTTPS connections via proxy. This default is identical on other platforms.
...
ProxyConfigServiceOHOS::ProxyConfigServiceOHOS(
    ...
    : delegate_(new Delegate(main_task_runner,
                             base::BindRepeating(&GetProperty))) {
  delegate_->FetchInitialConfig();

  OHOS::NWeb::OhosAdapterHelper::GetInstance()
  .GetNetProxyInstance().RegNetProxyEvent([this](std::string &host, uint16_t &port,
                                                const std::string &pac_url,
