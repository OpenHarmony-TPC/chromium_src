...
void DecodeTask(
  ...
  gpu::ImageDecodeAcceleratorWorker::CompletedDecodeCB scoped_decode_callback =
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(decode_cb),
                                                  nullptr);
  if (!decoder) {
    LOG(ERROR) << "[HeifSupport] DecodeTask: No decoder is available for supplied image";
    return;
  }
  ...
  // Due to the limitation of the surface system. OhosImageDecoderAdapter doesn't
  // provide a concurrent decoding mechanism. We make blocking calls from decoder
  // thread to main thread to ensure current decoding data has been processed
  // before a new decoding task starts.
  // io_thread                  main_thread               decode_thread
  //
