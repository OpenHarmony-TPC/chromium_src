...
class CodecEncodeBridgeCallback : public CodecCallbackAdapter,
...
public:
  static std::unique_ptr<OHOSMediaCodecBridge> CreateVideoEncoder(std::string &mime);
  ...
  CodecCodeAdapter Configure(const CodecConfigPara &config,
  ...
  CodecCodeAdapter ReleaseOutputBuffer(uint32_t index,
                                        bool render) override;
  void ClearKeyFrameCache() override;
  CodecCodeAdapter RequestKeyFrameSoon() override;
  CodecCodeAdapter FillSurfaceBuffer(scoped_refptr<VideoFrame> frame, const int64_t timestamp_ms) override;
