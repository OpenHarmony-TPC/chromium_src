 ...
 public:
  ...
  void HandleCrashNonFatal(int signo, siginfo_t* siginfo, void* context) {
    ...
    if (ConnectToHandler(signo, &connection) == 0) {
      LOG(INFO) << "crashpad SandboxedHandler::HandleCrashNonFatal, connect to handler successfully"\
 ...
 private:
  ...
  int ConnectToHandler(int signo, base::ScopedFD* connection) {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, fds) != 0) {
      LOG(ERROR) << "crashpad ConnectToHandler, create socketpair failed, errno = " << errno;
      return errno;
    }
    ...
    *reinterpret_cast<int*>(CMSG_DATA(cmsg)) = handlers_socket.get();
    ...
    if (HANDLE_EINTR(sendmsg(server_fd_, &msg, MSG_NOSIGNAL)) < 0) {
      LOG(ERROR) << "crashpad ConnectToHandler, sendmsg failed, errno = " << errno << ", handler fd = " << server_fd_;
      return errno;
    }
  ...
  static void HandleCrash(int signo, siginfo_t* siginfo, void* context) {
    LOG(INFO) << "crashpad SandboxedHandler::HandleCrash, received signo = " << signo;
...
class HandlerStarter {
 public:
  ...
  base::FilePath Initialize(bool dump_at_crash) {
    ...
    LOG(INFO) << "crashpad HandlerStarter::Initialize, crashpad report database path = " \
      << database_path << ", metrics path = " << metrics_path << ", upload url = " << url;
    base::FilePath exe_dir;
    base::FilePath handler_path;
    if (!GetHandlerPath(&exe_dir, &handler_path)) {
      LOG(ERROR) << "crash HandlerStarter::Initialize, get handler bin path failed";
    ...
    if (!base::PathExists(handler_path)) {
      LOG(ERROR) << "crashpad HandlerStarter::Initialize, handler path is not exist, handler path = " << handler_path;
    }
    ...
    if (!SetLdLibraryPath(exe_dir)) {
      LOG(ERROR) << "crash HandlerStarter::Initialize, set lib path failed, path = " << exe_dir;
      return database_path;
    }
  ...
  bool StartHandlerForClient(CrashReporterClient* client,
    ...
    LOG(INFO) << "crashpad HandlerStarter::StartHandlerForClient, crashpad report database path = " \
      << database_path << ", metrics path = " << metrics_path << ", upload url = " << url;
    base::FilePath exe_dir;
    base::FilePath handler_path;
    if (!GetHandlerPath(&exe_dir, &handler_path)) {
      LOG(ERROR) << "crash HandlerStarter::StartHandlerForClient, get handler bin path failed";
      return false;
    }
    ...
    if (!ShouldHandleCrashAndUpdateArguments(write_minidump_to_database,
                                             ...
                                             &arguments)) {
      LOG(ERROR) << "crash HandlerStarter::StartHandlerForClient, build handler bin args failed";
      return true;
    }

    if (!SetLdLibraryPath(exe_dir)) {
      LOG(ERROR) << "crash HandlerStarter::StartHandlerForClient, set lib path failed, path = " << exe_dir;
      return false;
    }
