...
#include "components/subresource_filter/content/browser/user_unindexed_ruleset_stream_generator.h"
...
std::unique_ptr<UserRulesetService> UserRulesetService::Create(
  ...
  base::FilePath indexed_ruleset_base_dir =
      user_data_dir.Append(kUserTopLevelDirectoryName)
...
UserRulesetService::UserRulesetService(
  ...
  publisher_ = publisher ? std::move(publisher)
                         : std::make_unique<UserRulesetPublisherImpl>(
                               this, blocking_task_runner);
  UserIndexedRulesetVersion most_recently_indexed_version;
  ...
  if (most_recently_indexed_version.IsValid() &&
    ...
    UserIndexedRulesetVersion().SaveToPrefs(local_state_);
  }
  DCHECK(publisher_->BestEffortTaskRunner()->BelongsToCurrentThread());
  publisher_->BestEffortTaskRunner()->PostTask(
      FROM_HERE,
      base::BindOnce(&UserRulesetService::FinishInitialization, AsWeakPtr()));
}
...
void UserRulesetService::IndexAndStoreAndPublishRulesetIfNeeded(
  ...
  IndexAndStoreRuleset(
      ...
      base::BindOnce(&UserRulesetService::OpenAndPublishRuleset, AsWeakPtr()));
}
...
UserIndexedRulesetVersion UserRulesetService::IndexAndWriteRuleset(
  ...
  if (!unindexed_ruleset_stream_generator.ruleset_stream()) {
    return UserIndexedRulesetVersion();
  }

  UserIndexedRulesetVersion indexed_version(
      unindexed_ruleset_info.content_version,
      UserIndexedRulesetVersion::CurrentFormatVersion());
...
UserRulesetService::IndexAndWriteRulesetResult UserRulesetService::WriteRuleset(
  ...
  if (base::WriteFile(UserIndexedRulesetLocator::GetRulesetDataFilePath(
                          scratch_dir.GetPath()),
...
void UserRulesetService::IndexAndStoreRuleset(
  ...
  background_task_runner_->PostTaskAndReplyWithResult(
      FROM_HERE,
      base::BindOnce(&UserRulesetService::IndexAndWriteRuleset,
                     indexed_ruleset_base_dir_, unindexed_ruleset_info),
      base::BindOnce(&UserRulesetService::OnWrittenRuleset, AsWeakPtr(),
...
void UserRulesetService::OpenAndPublishRuleset(
  ...
  publisher_->TryOpenAndSetRulesetFile(
      ...
      base::BindOnce(&UserRulesetService::OnRulesetSet, AsWeakPtr()));
}
