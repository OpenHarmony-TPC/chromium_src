...
std::vector<WebFormControlElement> GetFormControlElements(
  ...
  std::vector<WebFormControlElement> form_control_elements =
      element.GetFormControlElements().ReleaseVector();
  std::erase_if(form_control_elements,
                [](const WebFormControlElement& e) { return e.IsNull(); });
...
#if BUILDFLAG(ARKWEB_PASSWORD_AUTOFILL)
  void OnRequestAutofill(FormRendererId form_id,
    ...
    DeferMsg(&mojom::PasswordManagerDriver::OnRequestAutofill, form_id, state,
