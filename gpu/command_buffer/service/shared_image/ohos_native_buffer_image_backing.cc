...
gl::ScopedEGLImage CreateEGLImageFromNativeBuffer(OHOSNativeBuffer buffer) {
    ...
    int opstatus =
      OHOS::NWeb::OhosAdapterHelper::GetInstance().GetOhosNativeBufferAdapter().GetEGLBuffer(buffer, &eglBuffer);
    if (opstatus!=0) {
    ...
    OHOS::NWeb::OhosAdapterHelper::GetInstance().GetOhosNativeBufferAdapter().FreeEGLBuffer(eglBuffer);
...
OhosNativeBufferImageBacking::OhosNativeBufferImageBacking(const Mailbox& mailbox,
...
OhosNativeBufferImageBacking::OhosNativeBufferImageBacking(const Mailbox& mailbox,
  ...
  bool is_thread_safe,
  base::ScopedFD initial_upload_fd,
  bool use_passthrough): OhosImageBacking(mailbox,
                                    format,
                                    size,
                                    color_space,
                                    surface_origin,
...
OhosNativeBufferImageBacking::ProduceGLTexture(SharedImageManager* manager,
  ...
  auto egl_image =
      CreateEGLImageFromNativeBuffer(native_buffer_handle_.get());
  if (!egl_image.is_valid()) {
    LOG(ERROR) << "NATIVE_BUFFER ProduceGLTexture err";
    return nullptr;
  }

  // Right GL format for RGBX NativeBuffer is GL_RGB8, so we don't use angle rgbx.
...
OhosNativeBufferImageBacking::ProduceGLTexturePassthrough(
  ...
  auto egl_image =
      CreateEGLImageFromNativeBuffer(native_buffer_handle_.get());
  if (!egl_image.is_valid()) {
    LOG(ERROR) << "NATIVE_BUFFER "<< __PRETTY_FUNCTION__<< " :: " << __LINE__ << " !egl_image.is_valid()\n";
    return nullptr;
  }
...
OhosNativeBufferImageBacking::ProduceSkiaGanesh(
  ...
  if (context_state->GrContextIsVulkan()) {
    ...
    auto vulkan_image = CreateVkImageFromNativeBufferHandle(
        GetNativeBufferHandle(), context_state.get(), size(), format(), queue_family);
    if (!vulkan_image) {
