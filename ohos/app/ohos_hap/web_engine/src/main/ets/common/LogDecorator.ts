    ...
    (target: Object, methodName: string,
     propertyDescriptor: PropertyDescriptor): PropertyDescriptor => {
      ...
      propertyDescriptor.value = function(...args: object[]) {
        ...
        LogUtil.info(
            TAG, `${target.constructor.name}#${methodName}(${params}) in `);
        ...
        LogUtil.info(
            TAG,
            `${target.constructor.name}#${methodName}(${params}) out => ${r}`);
        return result;
      };
  ...
  Reflect.ownKeys(target.prototype).forEach(propertyKey => {
    ...
    if (method) {
      propertyDescriptor.value = function(...args: object[]) {
        const params = args.map(a => JSON.stringify(a)).join();
        LogUtil.info(
            TAG, `${target.name}#${propertyKey.toString()}(${params}) in `);
        ...
        LogUtil.info(
            TAG,
            `${target.name}#${propertyKey.toString()}(${params}) out => ${r}`);
        return result;
      };
