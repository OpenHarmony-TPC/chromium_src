...
void RenderAccessibilityImpl::PerformAction(const ui::AXActionData& data) {
  ...
  if(target->GetType() == ui::AXActionTarget::Type::kNull){
    ...
    while(curFrame && ilayers<iLayersMax){
      middleFrame = curFrame;
      if(middleFrame->ToWebLocalFrame()){
        document = middleFrame->ToWebLocalFrame()->GetDocument();
        target = AXActionTargetFactory::CreateFromNodeId(document, plugin_tree_source_,
                                              data.target_node_id);
        anchor = AXActionTargetFactory::CreateFromNodeId(document, plugin_tree_source_,
                                              data.anchor_node_id);
        focus = AXActionTargetFactory::CreateFromNodeId(document, plugin_tree_source_,
                                              data.focus_node_id);
      }else{
        break;
      }
