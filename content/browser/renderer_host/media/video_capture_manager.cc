...
media::VideoCaptureError VideoCaptureManager::StopScreenCapture(const std::string& sessionid) {

  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  std::lock_guard<std::mutex> lock(NWebIdMutex_);
  absl::optional<base::Token> token = base::Token::FromString(sessionid);
  if (!token.has_value()) {
