...
#if BUILDFLAG(ARKWEB_ENABLE_WISEPLAY)
void URLProvisionFetcher::RetrieveWiseplayLicense(const GURL& default_url,
    const std::string& request_data,
      ...
      net::DefineNetworkTrafficAnnotation("url_prevision_fetcher", R"(
        semantics {
          ...
          trigger:
            ...
            "sent to a provisioning server."
          data:
            "Opaque provision request generated by the CDM."
          destination: OTHER
        }
        policy {
          cookies_allowed: NO
  ...
  if (resource_request) {
    ...
    resource_request->headers.SetHeader("Accept-Encoding", "gzip, deflate");

    simple_url_loader_ = network::SimpleURLLoader::Create(
        std::move(resource_request), traffic_annotation);
    if (simple_url_loader_) {
      simple_url_loader_->AttachStringForUpload(request_data, "application/json");
...
void URLProvisionFetcher::OnSimpleLoaderComplete(
  ...
  if (response_body) {
    ...
    LOG(INFO) << "[DRM]" << __func__ << ", response size: " << response.size();
  } else {
    LOG(INFO) << "[DRM] CDM provision: server returned error code " << response_code;
  }
