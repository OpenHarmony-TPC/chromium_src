...
void GetOHOSCdmCapability(const std::string& key_system,
  ...
  // for now we only provide software decode
  const bool isSecure = robustness == CdmInfo::Robustness::kSoftwareSecure;
  if (!OhosMediaDrmBridge::IsKeySystemSupported(key_system)) {
    LOG(WARNING) << " [WiseplayDRM] Key system not support: " << key_system;
    std::move(cdm_capability_cb).Run(base::unexpected(
  ...
  for (const auto& codec : kWebMVideoCodecsToQuery) {
    if (OhosMediaCodecUtil::CanDecode(codec, isSecure)) {
  ...
  for (const auto& codec : kMP4VideoCodecsToQuery) {
    if (!capability.video_codecs.contains(codec) &&
        OhosMediaCodecUtil::CanDecode(codec, isSecure)) {
      capability.video_codecs.emplace(codec, kAllProfiles);
    }
  }

  if (isSecure && capability.video_codecs.empty()) {
    LOG(WARNING)
        << __FUNCTION__ << " [WiseplayDRM] Key system " << key_system
