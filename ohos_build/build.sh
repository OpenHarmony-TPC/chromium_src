...
${TEXT_BOLD}PRODUCT${TEXT_NORMAL}:
  wgr_64 musl_64 rk3568 rk3568_64 x86_64
  Default is: musl_64
"
}
...
while [ "$1" != "" ]; do
  case $1 in
    "wgr")
      echo " -> $1 <- is not been supported, please choose other products."
      usage
      exit 1
    ;;
    "wgr_64")
      ...
      build_product_name="product_name=\"wgr\""
    ;;
    "musl_64")
      buildarg_cpu="target_cpu=\"arm64\""
      buildarg_musl="use_musl=true"
...
if [ ${disable_pna_mode} -eq 1 ]; then
  GN_ARGS="${GN_ARGS} build_with_disable_pna_mode=true"
else
  GN_ARGS="${GN_ARGS} build_with_disable_pna_mode=false"
fi
