...
export default class LayoutUtils{
    ...
    static findMaxDimensions(root: HTMLElement) {
        let maxWidth = parseFloat(window.getComputedStyle(root).width);
        let maxHeight = parseFloat(window.getComputedStyle(root).height);
        function traverse(node: HTMLElement) {
            if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    // 根据原始约束关系，解除容器组件的限高，改成自适应
    @PerfExecution({ level: Level.INFO })
    static traversalFixWidth2() {
        Logger.printComInfo(`begin to run traversalFixWidth2`);
        for (const [comp, flag] of IntelligentLayout.HardCodeElementsCache)  {
            if(flag == true) continue;
            if (Utils.isPureText(comp) || Utils.isImageContainer(comp)) {
                continue;
            }
            if (comp.style.width.includes('px')) {
                Logger.printComInfo(`[调整宽度]${comp?.className}节点内联样式硬编码宽度值${comp.style.width},调整为100%`);
                // 设置了margin之后，宽度设置为100%，会有导致溢出的问题：余额宝的优选服务
                const containerWidth = parseFloat(getComputedStyle(comp.parentElement).width);
                // 减掉margin，就是子节点宽度值
                const width_ = containerWidth - parseFloat(getComputedStyle(comp).marginLeft) -  parseFloat(getComputedStyle(comp).marginRight);

                StyleSetter.setStyle(comp, Constant.width,  `${width_}px`);
                        // todo:FIXME：固定宽高的情况要特殊处理
                IntelligentLayout.HardCodeElementsCache.set(comp,true);
                continue;
            } else if (comp.style.width.includes('%')) {
                continue;
            }

            const widthValue = LayoutUtils.getDimensionRules(comp, Constant.width);
            if(widthValue.includes('rem')) {
                Logger.printComInfo(`[调整宽度]${comp?.className} width:${widthValue}`);
                // 计算展开态下的宽度
                const refWidth = (parseFloat(widthValue) * parseFloat(getComputedStyle(document.documentElement).fontSize))/Constant.scaleFactor;
                // 处理限制了固定宽高的
                const heightValue = LayoutUtils.getDimensionRules(comp, Constant.height);
                if(heightValue.includes('rem')) {
                    // 固定宽高，先不调整，FIXME:后面需要使用结构化算法，计算是否需要保持原有结构
                    Logger.printComInfo(`[调整宽度]${comp?.className} height:${heightValue}`);
                }

                // // 获取父容器的宽度
                const pWidth = parseFloat(getComputedStyle(comp.parentElement).width);
                Logger.printComInfo(`[调整宽度]${comp?.className} parent width:${pWidth}px`);
                if (refWidth > pWidth) {
                    StyleSetter.setStyle(comp, Constant.width, `${refWidth}px`);
                } else {
                    StyleSetter.setStyle(comp, Constant.width, `${pWidth}px`);
                }
        ...
        @PerfExecution({ level: Level.INFO })
        static FixImage() {
            for (const [comp, flag] of IntelligentLayout.imageCache)  {
                if(flag == true) continue;
                if (comp.style.width.includes('px')) {
                    Logger.printComInfo(`FixImage [调整宽度]${comp?.className}节点内联样式硬编码宽度值${comp.style.width},调整为100%`);
                    const containerWidth = parseFloat(getComputedStyle(comp).width);
                    const containerHeight = parseFloat(getComputedStyle(comp).height);
                    StyleSetter.setStyle(comp, Constant.width,  `${containerWidth}px`);
                    StyleSetter.setStyle(comp, Constant.height,  `${containerHeight}px`);
                            // todo:FIXME：固定宽高的情况要特殊处理
                    IntelligentLayout.imageCache.set(comp,true);
                    continue;
                } else if (comp.style.width.includes('%')) {
                    continue;
                }
                ...
                const heightValue = LayoutUtils.getDimensionRules(comp, Constant.height);
                if(widthValue.includes('rem')) {
                    Logger.printComInfo(`FixImage [调整宽度]${comp?.className} width:${widthValue}`);
                    // 计算展开态下的宽度
                    const refWidth = (parseFloat(widthValue) * parseFloat(getComputedStyle(document.documentElement).fontSize))/Constant.scaleFactor;
                    StyleSetter.setStyle(comp, Constant.width, `${refWidth}px`);
                    IntelligentLayout.imageCache.set(comp,true);
                ...
                if(heightValue.includes('rem')) {
                    Logger.printComInfo(`FixImage [调整高度]${comp?.className} width:${heightValue}`);
                    // 计算展开态下的宽度
                    const refHeight = (parseFloat(heightValue) * parseFloat(getComputedStyle(document.documentElement).fontSize))/Constant.scaleFactor;
                    StyleSetter.setStyle(comp, Constant.height, `${refHeight}px`);
                    IntelligentLayout.imageCache.set(comp,true);
                }
            }
        }
    ...
    // 根据原始约束关系，解除容器组件的限高，改成自适应
    @PerfExecution({ level: Level.INFO })
    static traversalFixHeight(components:Array<HTMLElement>) {
        if(!components || components.length == 0 ) return;

        components.forEach((comp) => {
            if(comp.childNodes.length == 0) {
                Logger.printComInfo("调整高度过程:无子节点,无需调整");
                return;
            }
            if(comp.style.height.includes('px')) {
                Logger.printComInfo("调整高度过程:节点内联样式硬编码高度值,调整为auto");
                StyleSetter.setStyle(comp, Constant.height,  `auto`);
                return;
            }
            const leaves = Array.from(comp.childNodes).filter(node => !Utils.shouldSkip(node as HTMLElement)) as Array<HTMLElement>;
            if(leaves) {
                LayoutUtils.traversalFixHeight(leaves);
            }
        });
    }
     ...
     // 递归遍历元素,调整宽度
     @PerfExecution({ level: Level.INFO })
     static traversalFixWidth(targetElement: HTMLElement, refElement:HTMLElement) {
        if (Utils.shouldSkip(targetElement) || Utils.isImageContainer(targetElement)) {
            return;
        }
        const refWidth = parseFloat(refElement.style.width);
        const targetWidth= parseFloat(window.getComputedStyle(targetElement).width);

        const refMarginLeft = parseFloat(refElement.style.marginLeft);
        const targetMarginLeft = parseFloat(window.getComputedStyle(targetElement).marginLeft);
        const refMarginright = parseFloat(refElement.style.marginRight);
        const targetMarginRight = parseFloat(window.getComputedStyle(targetElement).marginRight);
        const margin_diff = (refMarginLeft + refMarginright) -( targetMarginLeft + targetMarginRight);
        const margin_diff_fix = margin_diff > 0? margin_diff:0;

        if (Math.abs(refWidth - targetWidth) > 100) { // 前后的宽度偏差过大，使用原始值修复
            Logger.printComInfo(`根据原始宽度修复当前宽度: ${targetElement?.className}`);
            StyleSetter.setStyle(targetElement, Constant.width,  `${refWidth+ margin_diff_fix}px`);
        }
        const length = targetElement.children.length;
        for (let i = 0; i < length; i++) {
            const childTargetEle = targetElement.children[i] as HTMLElement;
            const childrefEle = refElement.children[i] as HTMLElement;
            LayoutUtils.traversalFixWidth(childTargetEle, childrefEle);
        }
    }

    static FontSizeAdapter() {
        if(LayoutUtils.hasInitFontSizeMediQuery) return;
        let fsize = parseFloat(getComputedStyle(document.querySelector("html"))?.fontSize);
        if(fsize == 0 ) {
            fsize = parseFloat(getComputedStyle(document.documentElement).fontSize);
        }
        fsize = fsize * Constant.scaleFactor;
        Logger.d(`FontSize设置为${fsize}px`);
        fsize = fsize <66 ? 66: fsize;
        const newStyle = document.createElement('style');
        newStyle.textContent = `@media only screen and (min-width: 710px) { html { font-size: ${fsize}px !important; } }`;
        // 将新的样式规则添加到文档中
        document.head.appendChild(newStyle);
        LayoutUtils.hasInitFontSizeMediQuery = true;
    }

    static getDimensionRules(element:HTMLElement, prop:string):string {
        // 获取元素的所有 CSS 规则
        if(LayoutUtils.hardSizeNodeCache.get(element)) {
            const v = LayoutUtils.hardSizeNodeCache.get(element).get(prop);
          ...
          try {
            for (const rule of sheet.cssRules) {
              if (rule instanceof CSSStyleRule && rule.selectorText && element.matches(rule.selectorText)) {
                const cssRule = rule.style.getPropertyValue(prop);
                if (cssRule && (cssRule.includes('rem') || cssRule.includes('px'))) {
                    if (parseFloat(cssRule) == 0) {
                        let propStyle = this.getPropFromStyle(element, prop);
                        let map = LayoutUtils.hardSizeNodeCache.get(element);
                        map = (map != null)?map: new Map<string,string>();
                        map.set(prop, propStyle);
                        LayoutUtils.hardSizeNodeCache.set(element, map);
                        return propStyle;
                    } else {
                        let map = LayoutUtils.hardSizeNodeCache.get(element);
                        map = (map != null)?map: new Map<string,string>();
                        map.set(prop,cssRule);
                        LayoutUtils.hardSizeNodeCache.set(element,map);
                        return cssRule;
                    }
                }
        ...
        // 检查内联样式
        let width_ = element.style.width;
        const height_ = element.style.height;
        if (width_ && (width_.includes('rem') || width_.includes('px'))) {
            let map = LayoutUtils.hardSizeNodeCache.get(element);
            map = (map != null)?map: new Map<string,string>();
            map.set(Constant.width, width_);
            LayoutUtils.hardSizeNodeCache.set(element,map);
        }
        if (height_ && (height_.includes('rem') || height_.includes('px'))) {
            let map = LayoutUtils.hardSizeNodeCache.get(element);
            map = (map != null)?map: new Map<string,string>();
            map.set(Constant.height, height_);
            LayoutUtils.hardSizeNodeCache.set(element,map);
        }
        if(prop == Constant.width) return width_;
        if(prop == Constant.height) return height_;
