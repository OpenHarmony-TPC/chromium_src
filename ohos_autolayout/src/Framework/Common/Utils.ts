...
export default class Utils {
    ...
    static isImageCanSafetyStrech(element:HTMLElement): boolean {
        ...
        if (bgImage && bgImage.includes('linear-gradient')) return true;
        return false;
    }
    ...
    static isImageContainer(element:HTMLElement): boolean {
        ...
        // 类型3：CSS背景图容器
        const bgImage = getComputedStyle(element).backgroundImage;
        if (bgImage && bgImage !== 'none' && bgImage !== 'initial') return true;

        // 类型4：CSS Mask图容器
        const maskImage = getComputedStyle(element).webkitMaskImage;
        if (maskImage && maskImage !== 'none' && maskImage.includes('url')) return true;
    ...
    static shouldSkip(ele: HTMLElement) :boolean {
        if (!ele || ele.nodeType !== Node.ELEMENT_NODE) return true;;
    ...
    static hasSemiTransparentGradient(gradientValue: string): boolean {
        if (!gradientValue || gradientValue === 'none') return false;
        
        // 注意：纯透明不算
        if (gradientValue.includes('transparent')) return false;
        
        // 检查RGBA/HSLA透明
        if (/rgba$$.+?$$|hsla$$.+?$$/i.test(gradientValue)) return true;
    ...
    static isBackgroundSemiTransparent(style:  CSSStyleDeclaration): boolean {
        ...
        if (Utils.isColorSemiTransparent(style.backgroundColor)) return true;
        ...
        // 4. 检查滤镜透明度
        if (Utils.hasSemiTransparentFilter(style.filter)) return true;

        // 5. 检查背景滤镜（毛玻璃效果）
        if (style.backdropFilter !== 'none') return true;

        // 6. 检查混合模式（可能产生透明叠加效果）
        if (/overlay|multiply|screen|soft-light/.test(style.mixBlendMode)) return true;
