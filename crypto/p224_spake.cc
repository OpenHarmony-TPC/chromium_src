...
P224EncryptedKeyExchange::P224EncryptedKeyExchange(PeerType peer_type,
  ...
  std::fill(std::begin(expected_authenticator_),
            std::end(expected_authenticator_), 0u);
#else
  ...
  SHA256HashString(std::string(password.data(), password.length()), pw_,
                   sizeof(pw_));
...
void P224EncryptedKeyExchange::CalculateHash(
  ...
  hash_contents += std::string(reinterpret_cast<const char*>(pw_), sizeof(pw_));
  hash_contents += k;
