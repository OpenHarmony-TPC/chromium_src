  ...
  auto read_callback = [&](int error_code, const std::vector<uint8_t>& value) {
    base::TaskRunnerOHOS::GetUIThreadTaskRunner()->PostTask(
        ...
        base::BindOnce(
            ...
            weak_ptr_factory_.GetWeakPtr(), error_code, std::move(value)));
  };
  BluetoothBleAttributeAdapter::ReadCharacteristic(identifier_, device_id_,
...
void BluetoothRemoteGattCharacteristicOHOS::WriteRemoteCharacteristic(
  ...
  BluetoothBleAttributeAdapter::WriteCharacteristic(
      identifier_, device_id_, static_cast<int>(gatt_write_type), value_vector,
  ...
  auto callback = [this](std::vector<DescriptorInfo> descriptor_infos) {
    base::TaskRunnerOHOS::GetUIThreadTaskRunner()->PostTask(
        ...
        base::BindOnce(
            ...
            weak_ptr_factory_.GetWeakPtr(), std::move(descriptor_infos)));
  };
  BluetoothBleAttributeAdapter::CreateDescriptor(identifier_, device_id_,
                                                 std::move(callback));
}
...
void BluetoothRemoteGattCharacteristicOHOS::SubscribeToNotifications(
  ...
  auto onChangeCallback = [this](int error_code,
    ...
    base::TaskRunnerOHOS::GetUIThreadTaskRunner()->PostTask(
        FROM_HERE,
        base::BindOnce(&BluetoothRemoteGattCharacteristicOHOS::SetValue,
                       weak_ptr_factory_.GetWeakPtr(), std::move(value)));
  };
  BluetoothBleAttributeAdapter::SubscribeToNotifications(
      identifier_, device_id_, std::move(onChangeCallback));
}
...
void BluetoothRemoteGattCharacteristicOHOS::UnsubscribeFromNotifications(
  ...
  BluetoothBleAttributeAdapter::UnsubscribeFromNotifications(
      identifier_, device_id_, std::move(UnsubscribeCallback));
}
}  // namespace device
