...
const std::unordered_map<Camera_Format, VideoPixelFormatAdapter> TO_ADAPTER_PIXEL_FORMAT_MAP = {
    ...
    { Camera_Format::CAMERA_FORMAT_YCBCR_P010, VideoPixelFormatAdapter::FORMAT_YCBCR_420_888 },
...
const std::unordered_map<VideoPixelFormatAdapter, Camera_Format> TO_OHOS_PIXEL_FORMAT_MAP = {
    ...
    { VideoPixelFormatAdapter::FORMAT_YCBCR_420_888, Camera_Format::CAMERA_FORMAT_YCBCR_P010 },
    ...
    Camera_Device& camera, Camera_OutputCapability* outputCapability)
{
    ...
    std::string displayName = GetCameraDisplayName(camera.cameraId, camera.cameraPosition);
...
int32_t CameraManagerAdapterImpl::GetCameraDeviceFromId(const std::string& deviceId, Camera_Device* camera)
{
    ...
    for (uint32_t i = 0; i < size; i++) {
        if (cameras[i].cameraId == deviceId) {
            for (uint32_t j = 0; j < MAX_CAMERA_ID_LEN && j < strlen(cameras[i].cameraId) + 1; j++) {
...
std::string CameraManagerAdapterImpl::GetCameraDisplayName(const std::string& cameraId, const Camera_Position& position)
{
    std::string displayName;
    switch (position) {
        case Camera_Position::CAMERA_POSITION_FRONT:
            displayName = cameraId + ", facing front";
            return displayName;
        case Camera_Position::CAMERA_POSITION_BACK:
            displayName = cameraId + ", facing back";
            return displayName;
        // ArkWeb_Media_Not_Implement: Camera_Position 不支持CAMERA_POSITION_FOLD_INNER
        default:
