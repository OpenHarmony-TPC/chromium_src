...
class NDKOhosAdapterHelper : public NWeb::OhosAdapterHelper {
 public:
  explicit NDKOhosAdapterHelper();

  std::unique_ptr<NWeb::AafwkAppMgrClientAdapter> CreateAafwkAdapter() override;

  std::unique_ptr<NWeb::PowerMgrClientAdapter> CreatePowerMgrClientAdapter()
      override;

  std::unique_ptr<NWeb::DisplayManagerAdapter> CreateDisplayMgrAdapter()
      override;

  std::unique_ptr<NWeb::BatteryMgrClientAdapter> CreateBatteryClientAdapter()
      override;
  ...
  std::unique_ptr<NWeb::AudioCapturerAdapter> CreateAudioCapturerAdapter()
      override;
  ...
  std::unique_ptr<NWeb::SocPerfClientAdapter> CreateSocPerfClientAdapter()
      override;

  std::unique_ptr<NWeb::OhosResourceAdapter> GetResourceAdapter(
      const std::string& hapPath) override;
  ...
  GetEnterpriseDeviceManagementInstance() override;
  ...
  std::unique_ptr<NWeb::IConsumerSurfaceAdapter> CreateConsumerSurfaceAdapter()
      override;
  ...
  std::unique_ptr<NWeb::ScreenCaptureAdapter> CreateScreenCaptureAdapter()
      override;

  std::unique_ptr<NWeb::DateTimeFormatAdapter> CreateDateTimeFormatAdapter()
      override;

  std::unique_ptr<NWeb::MediaCodecDecoderAdapter>
  CreateMediaCodecDecoderAdapter() override;

  std::unique_ptr<NWeb::NativeImageAdapter> CreateNativeImageAdapter() override;

  std::unique_ptr<NWeb::MediaCodecAdapter> CreateMediaCodecEncoderAdapter()
      override;
  ...
  CreateAudioCodecDecoderAdapter() override;
  ...
  NWeb::OhosDrawingTextTypographyAdapter& GetOhosDrawingTextTypographyAdapter()
      override;
