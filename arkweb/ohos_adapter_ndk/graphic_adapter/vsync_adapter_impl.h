...
public:
    VSyncAdapterNdkImpl() = default;
    ~VSyncAdapterNdkImpl() override;
    VSyncAdapterNdkImpl(const VSyncAdapterNdkImpl&) = delete;
    VSyncAdapterNdkImpl& operator=(const VSyncAdapterNdkImpl&) = delete;

    static VSyncAdapterNdkImpl& GetInstance();
...
private:
    static void OnVsync(long long timestamp, void* client);
    void VsyncCallbackInner(long long timestamp);
    VSyncErrorCode Init();
