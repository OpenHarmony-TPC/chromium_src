...
int32_t SensorTypeToOhosSensorType(int sensorTypeId, Sensor_Type *ohosSensorTypeId)
{
    ...
    WVLOG_E("Sendor type '%{public}d' is not supported", sensorTypeId);
...
void SensorAdapterImpl::OhosSensorCallback(Sensor_Event* event)
{
    ...
    switch (length) {
        case DATA_LEN_3:
            callback->UpdateOhosSensorData(
                timestamp, data[ARRAY_INDEX_0], data[ARRAY_INDEX_1], data[ARRAY_INDEX_2], 0.0f);
            break;
        case DATA_LEN_4:
            callback->UpdateOhosSensorData(
                timestamp, data[ARRAY_INDEX_0], data[ARRAY_INDEX_1], data[ARRAY_INDEX_2], data[ARRAY_INDEX_3]);
            break;
        default:
...
int32_t SensorAdapterImpl::SubscribeOhosSensor(int32_t sensorTypeId, int64_t samplingInterval)
{
    ...
    if (ret != SENSOR_SUCCESS) {
        WVLOG_E("SubscribeOhosSensor Error, SetCallback ret = %{public}d.", ret);
        return ret;
    }
    Sensor_SubscriptionId *id = OH_Sensor_CreateSubscriptionId();
    ret = OH_SensorSubscriptionId_SetType(id, ohosSensorTypeId);
    if (ret != SENSOR_SUCCESS) {
        WVLOG_E("SubscribeOhosSensor error, call SetType ret = %{public}d.", ret);
        return ret;
    }
    ...
    if (ret != SENSOR_SUCCESS) {
        WVLOG_E("SubscribeOhosSensor error, call SetSamplingInterval ret = %{public}d.", ret);
        return ret;
    }
    ret = OH_Sensor_Subscribe(id, attr, subscriber);
    if (ret != SENSOR_SUCCESS) {
        WVLOG_E("SubscribeOhosSensor error, call Subscribe ret = %{public}d.", ret);
        return ret;
    }
...
int32_t SensorAdapterImpl::UnsubscribeOhosSensor(int32_t sensorTypeId)
{
    ...
    if (ret == SENSOR_SUCCESS) {
        ...
        ret = OH_Sensor_Unsubscribe(sensorSubscriber->id, sensorSubscriber->subscriber);
        if (ret != SENSOR_SUCCESS) {
            WVLOG_E("UnsubscribeOhosSensor error, call Unsubscribe ret = %{public}d.", ret);
            return ret;
        }
