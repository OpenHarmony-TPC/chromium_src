    ...
    std::vector<InputMethod_PrivateCommand *> privateCommands)
{
    ...
    if (cJSON_HasObjectItem(sourceJson, "userName")) {
        ...
        cJSON* userName = cJSON_GetObjectItem(sourceJson, key.c_str());
        if (userName != nullptr && cJSON_IsString(userName) && userName->valuestring != nullptr) {
            ...
            InputMethod_ErrorCode ret = OH_PrivateCommand_SetStrValue(
    ...
    if (cJSON_HasObjectItem(sourceJson, "hasAccount")) {
        ...
        cJSON* hasAccount = cJSON_GetObjectItem(sourceJson, key.c_str());
    ...
    const std::shared_ptr<IMFTextConfigAdapter> config, bool isResetListener)
{
    ...
    if ((IMFTextEditorProxyImpl::textListener_ != nullptr) && isResetListener) {
