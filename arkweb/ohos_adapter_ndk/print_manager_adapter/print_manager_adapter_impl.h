...
public:
    ...
    static void OnStartLayoutWrite(const char *jobId, uint32_t fd, const Print_PrintAttributes *oldAttrs,
        const Print_PrintAttributes *newAttrs, Print_WriteResultCallback writeCallback);
...
public:
    explicit PrintWriteResultCallbackAdapterImpl(Print_WriteResultCallback cb);

    void WriteResultCallback(std::string jobId, uint32_t code) override;

private:
    Print_WriteResultCallback cb_;
};
