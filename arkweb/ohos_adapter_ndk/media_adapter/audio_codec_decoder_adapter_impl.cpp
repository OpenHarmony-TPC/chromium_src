...
static const std::unordered_map<OH_AVCodecBufferFlags, BufferFlag> BUFFER_FLAG_MAP = {
...
{
    WVLOG_I("outputBuffer[%{public}u] available, buffer size[%{public}d], pts[%{public}ld],"
    ...
    bool isEncrypted, BufferFlag flag)
{
    ...
    if (flag != GetBufferFlag(OH_AVCodecBufferFlags::AVCODEC_BUFFER_FLAGS_EOS)) {
    ...
    if (flag == GetBufferFlag(OH_AVCodecBufferFlags::AVCODEC_BUFFER_FLAGS_EOS)) {
        ...
        attr.flags = OH_AVCodecBufferFlags::AVCODEC_BUFFER_FLAGS_NONE;
    }
...
BufferFlag AudioCodecDecoderAdapterImpl::GetBufferFlag(OH_AVCodecBufferFlags codecBufferFlag)
{
