...
public:
    explicit DrmCallbackImpl(std::shared_ptr<DrmCallbackAdapter> callbackAdapter);
...
public:
    ...
    int32_t CloseSession(uint32_t promiseId, const std::string& emeId) override;
    int32_t RemoveSession(uint32_t promiseId, const std::string& emeId) override;
