...
NWebUserAgentMetadataImpl::NWebUserAgentMetadataImpl(
    const std::vector<std::string>& brand,
    const std::vector<std::string>& majorVersion,
    const std::vector<std::string>& brandFullVersion,
    const UserAgentMetadataInfo& info)
    : brand_(brand),
      majorVersion_(majorVersion),
      brandFullVersion_(brandFullVersion),
      arch_(info.arch),
      bitness_(info.bitness),
      formFactors_(info.formFactors),
      fullVersion_(info.fullVersion),
      isMobile_(info.isMobile),
      model_(info.model),
      platform_(info.platform),
      platformVersion_(info.platformVersion),
      isWow64_(info.isWow64) {}

void NWebUserAgentMetadataImpl::SetBrandVersionList(
    const std::vector<std::string>& brand,
    const std::vector<std::string>& majorVersion,
    const std::vector<std::string>& brandFullVersion) {
  if (brand.size() != majorVersion.size() ||
      brand.size() != brandFullVersion.size() ||
...
void NWebUserAgentMetadataImpl::SetFormFactors(
    const std::vector<std::string>& formFactors) {
  formFactors_.clear();
  std::set<std::string> formFactorSet{"Automotive", "Desktop", "Mobile", "EInk",
                                      "Tablet",     "Watch",   "XR"};
...
void NWebUserAgentMetadataImpl::SetPlatformVersion(
...
void NWebUserAgentMetadataImpl::GetBrandVersionList(
    ...
    std::vector<std::string>& majorVersion,
    std::vector<std::string>& brandFullVersion) {
  brand = brand_;
...
void NWebUserAgentMetadataImpl::SetBrandVersionListPtr(
    const std::vector<std::shared_ptr<NWebUserAgentBrandVersionImpl>>&
        brandVersionList) {
  brand_.clear();
  majorVersion_.clear();
...
std::vector<std::shared_ptr<NWebUserAgentBrandVersionImpl>>
NWebUserAgentMetadataImpl::GetBrandVersionListPtr() {
  std::vector<std::shared_ptr<NWebUserAgentBrandVersionImpl>> versions;
  for (size_t i = 0; i < brand_.size(); i++) {
    auto ptrBrandVersion =
        std::make_shared<OHOS::NWeb::NWebUserAgentBrandVersionImpl>(
