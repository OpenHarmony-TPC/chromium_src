...
enum ArkWeb_NetError {
  ...
  /*
   * @brief An asynchronous IO operation is not yet complete.  This usually does
   * not indicate a fatal error.  Typically this error will be generated as a
   *        notification to wait for some external notification that the IO
   * operation finally completed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief An unexpected error.  This may be caused by a programming mistake or
  ...
  /*
   * @brief The file upload failed because the file's modification time was
  ...
  /*
   * @brief The request was blocked by the URL block list configured by the
  ...
  /*
   * @brief The upload failed because the upload stream needed to be re-read,
   * due to a retry or a redirect, but the upload stream doesn't support that
   * operation.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The request failed because the URLRequestContext is shutting down,
  ...
  /*
   * @brief The request failed because the response was delivered along with
   * requirements which are not met ('X-Frame-Options' and
   * 'Content-Security-Policy' ancestor checks and
   * 'Cross-Origin-Resource-Policy' for instance,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The request was blocked by system policy disallowing some or all
   * cleartext requests.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief A connection timed out as a result of not receiving an ACK for data
  ...
  /*
   * @brief The IP address or port number is invalid (e.g., cannot connect to
   * the IP address 0 or the port 0,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The IP address is unreachable.  This usually means that there is no
  ...
  /*
   * @brief The server requested a client certificate for SSL client
   * authentication.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The proxy requested authentication (for tunnel establishment, with
  ...
  /*
   * @brief The SSL handshake failed because of a bad or missing client
   * certificate.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief There are too many pending DNS resolves, so a request in the queue
   * was aborted.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Failed establishing a connection to the SOCKS proxy server for a
   * target host.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The SOCKS proxy server failed establishing connection to the target
   * host because that host is unreachable.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Winsock sometimes reports more data written than passed.  This is
   * probably due to a broken LSP.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief An SSL peer sent us a fatal decompression_failure alert. This
   * typically occurs when a peer selects DEFLATE compression in the mistaken
   * belief that it supports it.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief An SSL peer sent us a fatal bad_record_mac alert. This has been
   * observed from servers with buggy DEFLATE support.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
   ...
   *        Note that this does NOT include failures during the actual "CONNECT"
   * method of an HTTP proxy.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief A mandatory proxy configuration could not be used. Currently this
   * means that a mandatory PAC script could not be fetched, parsed or executed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief We've hit the max socket limit for the socket pool while
   * preconnecting.  We don't bother trying to preconnect more sockets.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The permission to use the SSL client certificate's private key was
   * denied.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Permission to access the network was denied. This is used to
   * distinguish errors that were most likely caused by a firewall from other
   * access denied errors. See also ERR_ACCESS_DENIED.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
   ...
   *        received a 302 (temporary redirect, response.  The response body
  ...
  /*
   * @brief We were unable to sign the CertificateVerify data of an SSL client
   * auth handshake with the client certificate's private key. Possible causes
   * for this include the user implicitly or explicitly denying access to the
   * private key, the private key may not be valid for signing, the key may be
   * relying on a cached handle which is no longer valid, or the CSP won't allow
  ...
  /*
   * @brief The message was too large for the transport.  (for example a UDP
   * message which exceeds size threshold,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Websocket protocol error. Indicates that we are terminating the
   * connection due to a malformed frame or other protocol violation.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The certificate didn't match the built-in public key pins for the
   * host name. The pins are set in net/http/transport_security_state.cc and
   * require that one of a set of public keys exist on the path from the leaf to
   * the root.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Server request for client certificate did not contain any types we
   * support.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief An SSL peer sent us a fatal decrypt_error alert. This typically
   * occurs when a peer could not correctly verify a signature (in
   * CertificateVerify or ServerKeyExchange, or validate a Finished message.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief There are too many pending WebSocketJob instances, so the new job
   * was not pushed to the queue.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Failed to set the socket's receive buffer size as requested, despite
  ...
  /*
   * @brief Failed to set the socket's send buffer size as requested, despite
   * success return code from setsockopt.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Failed to import a client certificate from the platform store into
   * the SSL library.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   ...
   *        "127.0.53.53". This is a special IP address which ICANN has
   * recommended to indicate there was a name collision, and alert admins to a
   * potential problem.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The SSL server presented a certificate which could not be decoded.
   * This is not a certificate error code as no X509Certificate object is
   * available. This error is fatal.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Certificate Transparency: Received a signed tree head that failed to
   * parse.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Certificate Transparency: Received a signed tree head whose JSON
   * parsing was OK but was missing some of the fields.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The attempt to reuse a connection to send proxy auth credentials
   * failed before the AuthController was used to generate credentials. The
   * caller should reuse the controller with a new connection. This error is
   * only used internally by the network stack.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Certificate Transparency: Failed to parse the received consistency
   * proof.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The SSL server required an unsupported cipher suite that has since
   * been removed. This error will temporarily be signaled on a fallback for one
   * or two releases immediately following a cipher suite's removal, after which
   * the fallback will be removed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief When a WebSocket handshake is done successfully and the connection
   * has been upgraded, the URLRequest is cancelled with this error code.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Socket ReadIfReady support is not implemented. This error should not
   * be user visible, because the normal Read(, method is used as a fallback.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief There were no common signature algorithms between our client
   * certificate private key and the server's preferences.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief TLS 1.3 early data was rejected by the server. This will be received
   * before any data is returned from the socket. The request should be retried
   * with early data disabled.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief TLS 1.3 early data was offered, but the server responded with
   * TLS 1.2 or earlier. This is an internal error code to account for a
   *        backwards-compatibility issue with early data and TLS 1.2. It will
   * be received before any data is returned from the socket. The request should
   * be retried with early data disabled. See
   * https://tools.ietf.org/html/rfc8446#appendix-D.3 for details.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief TLS 1.3 was enabled, but a lower version was negotiated and the
   * server returned a value indicating it supported TLS 1.3. This is part of a
   * security check in TLS 1.3, but it may also indicate the user is behind a
   * buggy TLS-terminating proxy which implemented TLS 1.2 incorrectly. (See
  ...
  /*
   * @brief The server's certificate has a keyUsage extension incompatible with
   * the negotiated TLS key exchange method.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief ECH was enabled, the server was unable to decrypt the encrypted
   * ClientHello, and additionally did not present a certificate valid for the
   * public name.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The server responded with a certificate whose common name did not
   * match the host name.  This could mean:
  ...
  /*
   * @brief The server responded with a certificate that, by our clock, appears
   * to either not yet be valid or to have expired.  This could mean:
  ...
  /*
   * @brief The server responded with a certificate that is signed by an
   * authority we don't trust.  The could mean:
   *        1. An attacker has substituted the real certificate for a cert that
   *           contains their public key and is signed by their cousin.
   *        2. The server operator has a legitimate certificate from a CA we
   * don't know about, but should trust.
   ...
   *        NOTE: It's unclear how this differs from ERR_CERT_INVALID. For
   * consistency, use that code instead of this one from now on.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The certificate has no mechanism for determining if it is revoked.
   * In effect, this certificate cannot be revoked.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Revocation information for the security certificate for this site is
   * not available.  This could mean:
   *        1. An attacker has compromised the private key in the certificate
   * and is blocking our attempt to find out that the cert was revoked.
   *        2. The certificate is unrevoked, but the revocation server is busy
   * or unavailable.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   ...
   *        We have the capability to ignore this error, but it is probably not
   * the thing to do.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The server responded with a certificate that contains a weak key
   * (e.g. a too-small RSA key,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The certificate claimed DNS names that are in violation of name
   * constraints.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief Certificate Transparency was required for this connection, but the
   * server did not provide CT information that complied with the policy.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The certificate chained to a legacy Symantec root that is no longer
   * trusted.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The certificate is known to be used for interception by an entity
   * other the device owner.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Attempting to load an URL resulted in an unsafe redirect (e.g., a
   * redirect to file:// is considered unsafe,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The response was 407 (Proxy Authentication Required,, yet we did not
  ...
  /*
   * @brief The response was 416 (Requested range not satisfiable, and the
   * server cannot satisfy the range requested.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The server sent an FTP directory listing in a format we do not
   * understand.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief An HTTP Authentication scheme was tried which is not supported on
   * this machine.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief (GSSAPI, No Kerberos credentials were available during HTTP
   * Authentication.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief An unexpected, but documented, SSPI or GSSAPI status code was
   * returned.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The HTTP response contained multiple distinct Content-Length
   * headers.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief HTTP/2 headers have been received, but not all of them - status or
   * version headers are missing, so we're expecting additional frames to
   * complete them.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief No PAC URL configuration could be retrieved from DHCP. This can
   * indicate either a failure to retrieve the DHCP configuration, or that there
   * was no PAC URL configured in DHCP.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief HTTP/2 server refused the request without processing, and sent
   * either a GOAWAY frame with error code NO_ERROR and Last-Stream-ID lower
   * than the stream id corresponding to the request indicating that this
   * request has not been processed yet, or a RST_STREAM frame with error code
   * REFUSED_STREAM. Client MAY retry (on a different connection,.  See RFC7540
   * Section 8.1.4.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The HTTP response body transferred fewer bytes than were advertised
   * by the Content-Length header when the connection is closed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
   ...
   *        terminating zero-length chunk was never sent when the connection is
   * closed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The QUIC crypto handshake failed.  This means that the server was
   * unable to read any requests sent, so they may be resent.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The server was expected to return an HTTP/1.x response, but did not.
   * Rather than treat it as HTTP/0.9, this error is returned.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Received HTTP/2 RST_STREAM frame with NO_ERROR error code.  This
   * error should be handled internally by HTTP/2 code, and should not make it
   * above the SpdyStream layer.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief A pushed stream was claimed and later reset by the server. When this
   * happens, the request should be retried.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief An HTTP transaction was retried too many times due for
   * authentication or invalid certificates.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief A pushed HTTP/2 stream was claimed by a request based on matching
   * URL and request headers, but the pushed response headers do not match the
   * request.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief The certificate presented on a QUIC connection does not chain to a
   * known root and the origin connected to is not on a list of domains where
   * unknown roots are allowed.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief A GOAWAY frame has been received indicating that the request has not
   * been processed and is therefore safe to retry on a different connection.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The IP address space of the remote endpoint differed from the
   * previous observed value during the same request. Any cache entry for the
   * affected request should be invalidated.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The IP address space of the cached remote endpoint is blocked by
  ...
  /*
   * @brief Received a challenge after the transaction has read some data, and
   * the credentials aren't available.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief A Trust Tokens protocol operation-executing request failed for one
   * of a number of reasons (precondition failure, internal error, bad
   * response,.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
  ...
  /*
   * @brief When handling a Trust Tokens protocol operation-executing request,
   * the system was able to execute the request's Trust Tokens operation without
   * sending the request to its destination.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The server cannot fulfill the request at this point. This is a
   * temporary error. FTP response code 421.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The file is busy, or some other temporary error condition on opening
   * the file. FTP response code 450.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief Server rejected our command because we didn't issue the commands in
   * right order. FTP response code 503.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
   ...
   * query. 2 - Server failure - The name server was unable to process this
   * query due to a problem with the name server. 4 - Not Implemented - The name
   * server does not support the requested kind of query. 5 - Refused - The name
  ...
  /*
   * @brief The entry was not found in cache or other local sources, for lookups
   * where only local sources were queried.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief DNS identified the request as disallowed for insecure connection
   * (http/ws,. Error should be handled as if an HTTP redirect was received to
   * redirect to https or wss.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
  ...
  /*
   * @brief The hostname resolution of HTTPS record was expected to be resolved
   * with alpn values of supported protocols, but did not.
   *
   * @syscap SystemCapability.Web.Webview.Core
   * @since 12
   */
