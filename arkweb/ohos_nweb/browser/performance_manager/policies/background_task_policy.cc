...
void BackgroundTaskPolicy::OnIsMediaPlayingChanged(const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession page_node=" << page_node
  ...
  if (page_node) {
    ...
    // when current page to end of media
             ...
             IsEndOfMedia(page_node) && !is_main_frame_url_changed_) {
      bool ret = false;
      SetWebviewShow(page_node, false, false, ret);
      if (ret) {
        last_avsession_page_node_ = nullptr;
  ...
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << ", media avsession OnIsMediaPlayingChanged "
            << (page_node->IsMediaPlaying() ? "true" : "false")
            << ", page_node=" << page_node
...
bool BackgroundTaskPolicy::IsControllable(const PageNode* page_node) {
  bool ret = false;
  if (page_node) {
    auto webcontents = page_node->GetWebContents().get();
    if (webcontents) {
...
bool BackgroundTaskPolicy::IsPauseByAvsession(const PageNode* page_node) {
  ...
  auto web_contents = page_node->GetWebContents().get();
  if (!web_contents) {
    return ret;
  }
...
// While press backward button, the page will change to a new payge, the Url
// will change, so this function will be called
...
void BackgroundTaskPolicy::OnOpenerFrameNodeChanged(
    const PageNode* page_node,
    const FrameNode* previous_opener) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}
...
void BackgroundTaskPolicy::OnLoadingStateChanged(
    const PageNode* page_node,
...
void BackgroundTaskPolicy::OnPageLifecycleStateChanged(
    const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}

void BackgroundTaskPolicy::OnPageIsHoldingWebLockChanged(
    const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}

void BackgroundTaskPolicy::OnPageIsHoldingIndexedDBLockChanged(
    const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}

void BackgroundTaskPolicy::OnMainFrameDocumentChanged(
    const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}
void BackgroundTaskPolicy::OnHadFormInteractionChanged(
    const PageNode* page_node) {
  LOG(INFO) << BG_TASK_TAG << __FUNCTION__
            << " media avsession in out page_node=" << page_node;
}
