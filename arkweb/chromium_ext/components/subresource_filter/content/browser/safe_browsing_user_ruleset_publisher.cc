...
std::unique_ptr<UserRulesetPublisher> SafeBrowsingUserRulesetPublisher::Factory::Create(
    UserRulesetService* ruleset_service,
    scoped_refptr<base::SequencedTaskRunner> blocking_task_runner) const {
  return std::make_unique<SafeBrowsingUserRulesetPublisher>(
      ruleset_service, std::move(blocking_task_runner));
}

void SafeBrowsingUserRulesetPublisher::UserSendRulesetToRenderProcess(
    base::File* file,
