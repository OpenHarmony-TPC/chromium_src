...
UserRulesetPublisher::UserRulesetPublisher(
...
void UserRulesetPublisher::SetRulesetPublishedCallbackForTesting(
    base::OnceClosure callback) {
  ruleset_published_callback_ = std::move(callback);
}

void UserRulesetPublisher::TryOpenAndSetRulesetFile(
...
void UserRulesetPublisher::PublishNewRulesetVersion(
  ...
  for (auto it = content::RenderProcessHost::AllHostsIterator(); !it.IsAtEnd();
       it.Advance()) {
    UserSendRulesetToRenderProcess(ruleset_data_.get(), it.GetCurrentValue());
  }
...
UserRulesetPublisher::BestEffortTaskRunner() {
  return best_effort_task_runner_;
}

VerifiedRulesetDealer::Handle* UserRulesetPublisher::GetRulesetDealer() {
...
void UserRulesetPublisher::OnRenderProcessHostCreated(
    content::RenderProcessHost* rph) {
  if (!ruleset_data_ || !ruleset_data_->IsValid())
    return;
  UserSendRulesetToRenderProcess(ruleset_data_.get(), rph);
}
