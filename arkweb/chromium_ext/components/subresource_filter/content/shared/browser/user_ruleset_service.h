...
// Based on ruleset_service.h originally written by
// Copyright 2016 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file contains the top-level class for the UserRulesetService.  There
// are associated classes that tie this into the dealer as well as the filter
...
#ifndef COMPONENTS_SUBRESOURCE_FILTER_CONTENT_SHARED_BROWSER_USER_RULESET_SERVICE_H_
#define COMPONENTS_SUBRESOURCE_FILTER_CONTENT_SHARED_BROWSER_USER_RULESET_SERVICE_H_
...
class UserRulesetServiceClient {
 public:
...
//           \...
//
class UserIndexedRulesetLocator {
 public:
...
class UserRulesetService {
 public:
  ...
  static std::unique_ptr<UserRulesetService> Create(
  ...
  // initialization of the UserRulesetService, performing tasks that won't
  // slow down Chrome startup, then queues the FinishInitialization task. NOTE:
  // This constructor supports specifying various params explicitly for tests.
  // Production code should favor UserRulesetService::Create().
  UserRulesetService(
      const RulesetConfig& config,
      PrefService* local_state,
  ...
  UserIndexedRulesetVersion GetMostRecentlyIndexedVersion() const;
 ...
 private:
  ...
  using WriteRulesetCallback =
      base::OnceCallback<void(const UserIndexedRulesetVersion&)>;
  ...
  static UserIndexedRulesetVersion IndexAndWriteRuleset(
      const RulesetConfig& config,
      const base::FilePath& indexed_ruleset_base_dir,
      const UserUnindexedRulesetInfo& unindexed_ruleset_info);
  ...
  // UserRulesetService is considered initialized.
  ...
  void OpenAndPublishRuleset(const UserIndexedRulesetVersion& version);
  ...
  UserUnindexedRulesetInfo queued_unindexed_ruleset_info_;
  bool is_initialized_;

  const base::FilePath indexed_ruleset_base_dir_;
  const base::FilePath unindexed_ruleset_base_dir_;
  const raw_ptr<UserRulesetServiceClient> ruleset_service_client_;

  base::WeakPtrFactory<UserRulesetService> weak_ptr_factory_{this};
};

}  // namespace subresource_filter

#endif  // COMPONENTS_SUBRESOURCE_FILTER_CONTENT_SHARED_BROWSER_USER_RULESET_SERVICE_H_
