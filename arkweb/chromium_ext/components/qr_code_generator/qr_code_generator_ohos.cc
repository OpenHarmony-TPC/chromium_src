...
#include "components/qr_code_generator/qr_code_generator_ohos.h"
...
void QRCodeGenerator::fillAt(int x, int y, size_t len, uint8_t value) {
  ...
  DCHECK_LT(y, version_info_->size);
#if BUILDFLAG(ARKWEB_SAFE_FUNCTION)
  size_t offset = version_info_->size * y + x;
  size_t destMax = version_info_->size * version_info_->size - offset;
  memset_s(&d_[offset], destMax, value, len);
#else
...
void QRCodeGenerator::copyTo(int x, int y, const uint8_t* data, size_t len) {
  ...
  DCHECK_LT(y, version_info_->size);
#if BUILDFLAG(ARKWEB_SAFE_FUNCTION)
  memcpy_s(&d_[version_info_->size * y + x], len, data, len);
#else
...
#if BUILDFLAG(ARKWEB_SAFE_FUNCTION)
  if (memset_s(remainder, sizeof(remainder), 0, block_ec_bytes) != EOK) {
    LOG(ERROR) << "AddErrorCorrection memset_s failed.";
    return;
  }
#else
...
#if BUILDFLAG(ARKWEB_SAFE_FUNCTION)
  if (memmove_s(&out[0], block_data_bytes, &in[0], block_data_bytes) != EOK) {
    LOG(ERROR) << "AddErrorCorrection memmove_s failed.";
    return;
  }
#else
...
base::expected<GeneratedCode, Error> GenerateCode(
    ...
    std::optional<int> min_version) {
  QRCodeGenerator qr;
  return qr.Generate(in, min_version);
}
