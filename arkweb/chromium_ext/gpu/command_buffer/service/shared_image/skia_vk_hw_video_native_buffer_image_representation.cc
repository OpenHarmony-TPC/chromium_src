    ...
    std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
  ...
  TRACE_EVENT0("gpu", "SkiaVkHWVideoNBImageRepresentation::BeginWriteAccess");
...
std::vector<sk_sp<GrPromiseImageTexture>> SkiaVkHWVideoNBImageRepresentation::BeginWriteAccess(
    std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores,
    std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
  ...
  *end_state = GetEndAccessState();
  ...
  if (!promise_texture_) {
    LOG(ERROR) << "Promise texture is null in BeginWriteAccess (PromiseImageTexture).";
    return {};
  }
    ...
    std::unique_ptr<skgpu::MutableTextureState>* end_state)
{
  ...
  DCHECK(!surface_);
  DCHECK(promise_texture_);
  TRACE_EVENT0("gpu", "SkiaVkHWVideoNBImageRepresentation::BeginReadAccess");
  ...
  *end_state = GetEndAccessState();
  ...
  if (!promise_texture_) {
    LOG(ERROR) << "Promise texture is null in BeginReadAccess.";
    return {};
  }
  return {promise_texture_};
}
...
gpu::VulkanImplementation* SkiaVkHWVideoNBImageRepresentation::vk_implementation()
{
  return context_state_->vk_context_provider()->GetVulkanImplementation();
}

VkDevice SkiaVkHWVideoNBImageRepresentation::vk_device()
{
...
VkPhysicalDevice SkiaVkHWVideoNBImageRepresentation::vk_phy_device()
{
  return context_state_->vk_context_provider()
      ->GetDeviceQueue()
...
VkQueue SkiaVkHWVideoNBImageRepresentation::vk_queue()
{
  return context_state_->vk_context_provider()
      ->GetDeviceQueue()
      ->GetVulkanQueue();
    ...
    base::ScopedFD init_read_fence)
{
  ...
  TRACE_EVENT0("gpu", "SkiaVkHWVideoNBImageRepresentation::BeginAccess");

  // Synchronise the read access with the writes.
  base::ScopedFD sync_fd;
  if (readonly) {
    if (!hw_ohos_backing()->BeginRead(this, &sync_fd)) {
      return false;
    }
  } else {
    if (!hw_ohos_backing()->BeginWrite(&sync_fd)) {
      return false;
    }
  }
...
void SkiaVkHWVideoNBImageRepresentation::EndAccess(bool readonly)
{
  ...
  if (readonly) {
    hw_ohos_backing()->EndRead(this, std::move(sync_fd));
  } else {
    hw_ohos_backing()->EndWrite(std::move(sync_fd));
  }

  std::vector<VkSemaphore> semaphores;
  semaphores.reserve(kSemaphoresVectorSize);
  if (begin_access_semaphore_ != VK_NULL_HANDLE) {
...
std::unique_ptr<skgpu::MutableTextureState> SkiaVkHWVideoNBImageRepresentation::GetEndAccessState()
{
