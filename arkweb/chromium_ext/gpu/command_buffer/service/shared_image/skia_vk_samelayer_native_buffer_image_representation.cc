...
SkiaVkSamelayerNBImageRepresentation::SkiaVkSamelayerNBImageRepresentation(
...
std::vector<sk_sp<SkSurface>> SkiaVkSamelayerNBImageRepresentation::BeginWriteAccess(
...
SkiaVkSamelayerNBImageRepresentation::BeginWriteAccess(
    std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores,
    std::unique_ptr<skgpu::MutableTextureState>* end_state) {
  DCHECK_EQ(mode_, RepresentationAccessMode::kNone);
...
void SkiaVkSamelayerNBImageRepresentation::EndWriteAccess() {
...
std::vector<sk_sp<GrPromiseImageTexture>>
SkiaVkSamelayerNBImageRepresentation::BeginReadAccess(
    std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores,
    std::unique_ptr<skgpu::MutableTextureState>* end_state) {
  DCHECK_EQ(mode_, RepresentationAccessMode::kNone);
...
void SkiaVkSamelayerNBImageRepresentation::EndReadAccess() {
...
gpu::VulkanImplementation* SkiaVkSamelayerNBImageRepresentation::vk_implementation() {
  return context_state_->vk_context_provider()->GetVulkanImplementation();
}

VkDevice SkiaVkSamelayerNBImageRepresentation::vk_device() {
  return context_state_->vk_context_provider()
      ->GetDeviceQueue()
      ->GetVulkanDevice();
}

VkPhysicalDevice SkiaVkSamelayerNBImageRepresentation::vk_phy_device() {
  return context_state_->vk_context_provider()
      ->GetDeviceQueue()
      ->GetVulkanPhysicalDevice();
}

VkQueue SkiaVkSamelayerNBImageRepresentation::vk_queue() {
  return context_state_->vk_context_provider()
      ->GetDeviceQueue()
      ->GetVulkanQueue();
}

bool SkiaVkSamelayerNBImageRepresentation::BeginAccess(
    bool readonly,
    std::vector<GrBackendSemaphore>* begin_semaphores,
    std::vector<GrBackendSemaphore>* end_semaphores,
  ...
  if (readonly) {
    if (!ohos_backing()->BeginRead(this, &sync_fd)) {
      return false;
    }
  } else {
...
void SkiaVkSamelayerNBImageRepresentation::EndAccess(bool readonly) {
  base::ScopedFD sync_fd;
  ...
  semaphores.reserve(semaphoresNum);
  if (begin_access_semaphore_ != VK_NULL_HANDLE) {
...
SkiaVkSamelayerNBImageRepresentation::GetEndAccessState() {
