...
class VisitStringTableInfo : public v8::internal::RootVisitor {
  public:
    explicit VisitStringTableInfo(v8::internal::Heap* heap, BinaryWriterBase* writer)
        : heap_(heap),
    ...
    void VisitRootPointers(v8::internal::Root root, const char* description,
    ...
    void VisitRootPointers(v8::internal::Root root, const char* description,
      ...
      for (v8::internal::OffHeapObjectSlot p = start; p < end; ++p) {
        ...
        if (IsHeapObject(obj)) {
          v8::internal::Tagged<v8::internal::HeapObject> heap_object = Cast<v8::internal::HeapObject>(obj);
          dfx::Address object_address = heap_object->address();
          ...
          writer_->WriteBinBlock(reinterpret_cast<uint8_t*>(&len),
                                 sizeof(len));
        }
      }
    }

    uint32_t GetTotalObjectSize() {
