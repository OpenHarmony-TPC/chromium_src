...
#define LOGE(fmt, args...)                                                 \
  printf("E %s:%d  %s - " fmt, basename(__FILE__), __LINE__, __FUNCTION__, \
         ##args)
#define LOGI(fmt, args...) \
  printf("I %s:%d - " fmt, __FUNCTION__, __LINE__, ##args)
#define LOGW(fmt, args...)                                                 \
  printf("W %s:%d  %s - " fmt, basename(__FILE__), __LINE__, __FUNCTION__, \
         ##args)
...
struct GenericFamily {
  ...
  std::shared_ptr<TypefaceSet>
      typefaceSet;  // the typeface set of the font style set
...
class FontConfig_OHOS {
 ...
 private:
  ...
  using VariationMap =
      skia_private::THashMap<SkString, std::vector<VariationInfo>>;
  using TtcIndexMap = skia_private::THashMap<SkString, TtcIndexInfo>;
  SkFontScanner_FreeType newfontScanner;  // the scanner to parse a font file
  ...
  struct AliasInfo {
    ...
    int weight;  // the weight of the font style set. 0 means no specified
                 // weight
  };
  ...
  NamesMap
      genericNames;  // a map to store the index of a family for generic family
  NamesMap fallbackNames;  // a map to store the index of a family for fallback
                           // family
