 ...
 * \note   The caller must call unref() on the returned object if it's not null
 */
sk_sp<SkFontStyleSet> SkFontMgr_OHOS::onMatchFamily(
  ...
  return sk_sp<SkFontStyleSet>(
      new SkFontStyleSet_OHOS(fontConfig, index, isFallback));
}
 ...
 * \n      Return the typeface in the default font style set, if family name is
 * null \n      Return null, if family name is not found \note   The caller must
 * call unref() on the returned object if it's not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onMatchFamilyStyle(
  ...
  if (familyName) {
    ...
    if (styleIndex < 0 && SkString(familyName) == SkString("sans-serif")) {
      ...
      styleIndex =
          fontConfig->getStyleIndex(sansFamilyName.c_str(), isFallback);
    }
  }
 ...
 * \return Return the typeface in the default fallback set, if familyName is
 * null \return Return null, if the typeface is not found for the given
 * character \note The caller must call unref() on the returned object if it's
 * not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onMatchFamilyStyleCharacter(
    const char familyName[],
  ...
  while (true) {
    if (bcp47Count > 0) {
      ...
      for (unsigned int i = item->index;
           i < item->index + item->count && i < fallbackSet.size(); i++) {
...
SkTypeface* SkFontMgr_OHOS::findTypeface(const FallbackSetPos& fallbackItem,
  ...
  const FallbackSet& fallbackSet = fontConfig->getFallbackSet();
  // example bcp47 code : 'zh-Hans' : ('zh' : iso639 code, 'Hans' : iso15924
  // code) iso639 code will be taken from bcp47 code, so that we can try to
  // match bcp47 or only iso639. Therefore totalCount need to be 'bcp47Count *
  // 2'
 ...
 * \param bcp47Count the array size of bcp47
 * \param tps an array of the index of typeface which is matching one value of
 * bcp47 \return The index of language in bcp47, if matching happens \n Return
 * -1, if no language matching happens
 */
  ...
  for (int i = bcp47Count - 1; i >= 0; i--) {
    ...
    if (langs.find(bcp47[i]) != -1) {
      ...
      if (langs.find(iso15924) != -1 ||
          (strncmp(bcp47[i], "und", strlen("und")) &&
           langs.find(country.c_str()) != -1)) {
        return i + bcp47Count;
      }
    }
  }
 ...
 * \note The caller must call unref() on the returned object if it's not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onMakeFromData(sk_sp<SkData> data,
                                                 int ttcIndex) const {
  if (data == nullptr) {
    return nullptr;
  }
  std::unique_ptr<SkMemoryStream> memoryStream =
 ...
 * \note The caller must call unref() on the returned object if it's not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onMakeFromStreamIndex(
 ...
 * \return The object of typeface, if successful
 * \n      Return null if the font file is not found or the content of file is
 * not recognized. \note The caller must call unref() on the returned object if
 * it's not null
 */
 ...
 * \return The object of typeface which is the closest matching 'style' when the
 * familyName is found \return Return a typeface from the default family, if
 * familyName is not found \return Return null, if there is no any typeface in
 * the system \note The caller must caller unref() on the returned object is
 * it's not null
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::onLegacyMakeTypeface(
    const char familyName[],
 ...
 * \n      Return null, if the stream is not recognized
 */
sk_sp<SkTypeface> SkFontMgr_OHOS::makeTypeface(
    std::unique_ptr<SkStreamAsset> stream,
