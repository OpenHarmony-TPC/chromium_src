...
#include "arkweb/chromium_ext/third_party/skia/include/ports/SkFontConfig_ohos.h"
 ...
 */
int FontConfig_OHOS::parseAlias(const Json::Value& root,
                                std::vector<AliasInfo>& aliasSet) {
    if (root.empty()) {
 ...
 */
int FontConfig_OHOS::parseAdjust(const Json::Value& root,
                                 std::vector<AdjustInfo>& adjustSet) {
 ...
 * \return ERROR_CONFIG_INVALID_VALUE_TYPE invalid value type for an attribute
 */
int FontConfig_OHOS::parseTtcIndex(const Json::Value& root,
...
void FontConfig_OHOS::getAxisValues(const AxisDefinitions& axisDefs,
    ...
    SkFontScanner_FreeType::computeAxisValues(axisDefs, position,
        axisValues, font.familyName, &font.style);
 ...
 * \param fontScanner a scanner used to parse the font file
...
int FontConfig_OHOS::scanFonts(const SkFontScanner& fontScanner) {
    ...
    for (unsigned int i = 0; i < fontDirSet.size(); i++) {
        ...
        while ((node = readdir(dir))) {
            ...
            char fullname[len];
#if BUILDFLAG(ARKWEB_SAFE_FUNCTION)
            if (memset_s(fullname, len, 0, len) != EOK ||
                strcpy_s(fullname, sizeof(fullname), fontDirSet[i].c_str()) != EOK ||
                (fontDirSet[i][fontDirSet[i].size() - 1] != '/' &&
                 strcat_s(fullname, sizeof(fullname), "/") != EOK) ||
                strcat_s(fullname, sizeof(fullname), fname) != EOK) {
                continue;
            }
#else
...
void FontConfig_OHOS::resetGenericValue() {
    aliasMap.foreach([this](const SkString& key,
                            std::vector<AliasInfo>* pAliasSet) {
...
void FontConfig_OHOS::buildSubTypefaceSet(const std::shared_ptr<TypefaceSet>& typefaceSet,
                                          ...
                                          const SkString& familyName, int weight) {
 ...
 * \return err
 */
int FontConfig_OHOS::logErrInfo(int err, const char* key,
