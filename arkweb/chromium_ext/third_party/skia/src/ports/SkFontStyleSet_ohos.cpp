 ...
 */
SkFontStyleSet_OHOS::SkFontStyleSet_OHOS(
    const std::shared_ptr<FontConfig_OHOS>& fontConfig,
    int index,
    bool isFallback)
...
void SkFontStyleSet_OHOS::getStyle(int index,
  ...
  SkTypeface* typeface =
      fontConfig_->getTypeface(styleIndex, index, isFallback);
  if (typeface == nullptr) {
    return;
  }
  ...
  if (styleName) {
    ...
    // the value of font weight is between 0 ~ 1000 (refer to
    // SkFontStyle::Weight) the weight is divided by 100 to get the matched name
