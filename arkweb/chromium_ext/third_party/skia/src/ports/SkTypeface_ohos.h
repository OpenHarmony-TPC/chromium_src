...
class SkTypeface_OHOS : public SkTypeface_FreeType {
 ...
 protected:
  virtual std::unique_ptr<SkStreamAsset> onOpenStream(
  ...
  virtual sk_sp<SkTypeface> onMakeClone(
      const SkFontArguments& args) const override;

 private:
  SkString specifiedName;  // specified family name which is defined in the
                           // configuration file
