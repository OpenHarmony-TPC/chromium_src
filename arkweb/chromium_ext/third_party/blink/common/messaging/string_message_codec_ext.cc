...
void WriteString(const std::string& str, std::vector<uint8_t>* buffer) {
  ...
  blink::WriteUint32(str.length(), buffer);
  blink::WriteBytes(str_as_bytes, buffer);
}
...
void WriteDouble(const double& value, std::vector<uint8_t>* buffer) {
  ...
  const size_t doubleByteSize = sizeof(double);
  base::span<const uint8_t> value_as_bytes(
      reinterpret_cast<const uint8_t*>(&value), doubleByteSize);
  WriteBytes(value_as_bytes, buffer);
}
