...
class MediaControlEnteredFullScreenTitleDisplayExementExtTest
    : public testing::Test {
 protected:
  void SetUp() override {
  ...
  Persistent<MediaControlEnteredFullscreenTitleDisplayElement> elem_;
};

TEST_F(MediaControlEnteredFullScreenTitleDisplayExementExtTest, SetTitle1) {
  String initial_title = "Test Title";
  elem_->title_ = initial_title;
  elem_->SetTitle(initial_title);
  EXPECT_EQ(elem_->title_, initial_title);
}

TEST_F(MediaControlEnteredFullScreenTitleDisplayExementExtTest, SetTitle2) {
  elem_->title_ = "non-empty";
  String new_title = "";
  elem_->SetTitle(new_title);
  EXPECT_EQ(elem_->title_, new_title);
}

TEST_F(MediaControlEnteredFullScreenTitleDisplayExementExtTest, SetTitle3) {
  elem_->title_ = "prev";
  String new_title = "1234?! ";
  elem_->SetTitle(new_title);
  EXPECT_EQ(elem_->title_, new_title);
}

TEST_F(MediaControlEnteredFullScreenTitleDisplayExementExtTest, SetTitle4) {
  const Vector<String> titles = {
      String(u"\u05D0abc"),  // kRightToLeft
      String(u"\u0645xyz"),  // kRightToLeftArabic
      String(u"\u202B1"),    // kRightToLeftEmbedding
      String(u"\u202E1"),    // kRightToLeftOverride
      String("A123"),        // kLeftToRight
      String(u"\u202A1"),    // kLeftToRightEmbedding
      String(u"\u202D1")     // kLeftToRightOverride
  };

  for (const auto& t : titles) {
    elem_->title_ = "prev";
    elem_->SetTitle(t);
    EXPECT_EQ(elem_->title_, t);
  }
}
