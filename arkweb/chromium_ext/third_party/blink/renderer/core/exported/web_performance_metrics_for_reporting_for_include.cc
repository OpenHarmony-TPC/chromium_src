...
double WebPerformanceMetricsForReporting::RedirectEnd() const {
  return base::Milliseconds(private_->timing()->redirectEnd()).InSecondsF();
}

double WebPerformanceMetricsForReporting::FetchStart() const {
  return base::Milliseconds(private_->timing()->fetchStart()).InSecondsF();
}

double WebPerformanceMetricsForReporting::ResponseEnd() const {
  return base::Milliseconds(private_->timing()->responseEnd()).InSecondsF();
}
