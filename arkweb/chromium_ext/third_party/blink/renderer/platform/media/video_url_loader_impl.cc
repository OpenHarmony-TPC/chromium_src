...
class VideoURLLoaderImpl::ClientAdapter final
    : public GarbageCollected<ClientAdapter>,
      public ThreadableLoaderClient {
 public:
  ClientAdapter(VideoURLLoaderImpl*,
                WebAssociatedURLLoaderClient*,
...
VideoURLLoaderImpl::ClientAdapter::ClientAdapter(
    VideoURLLoaderImpl* loader,
...
bool VideoURLLoaderImpl::ClientAdapter::WillFollowRedirect(
    uint64_t identifier,
    const KURL& new_url,
...
void VideoURLLoaderImpl::ClientAdapter::DidSendData(
    uint64_t bytes_sent,
...
void VideoURLLoaderImpl::ClientAdapter::DidReceiveResponse(
    uint64_t,
...
void VideoURLLoaderImpl::ClientAdapter::DidDownloadData(uint64_t data_length) {
  if (!client_) {
    return;
  }
...
void VideoURLLoaderImpl::ClientAdapter::DidReceiveData(base::span<const char> data_length) {
  if (!client_) {
    return;
  }
  client_->DidReceiveData(data_length);
}

void VideoURLLoaderImpl::ClientAdapter::DidFinishLoading(uint64_t identifier) {
  if (!client_) {
    return;
  }
...
void VideoURLLoaderImpl::ClientAdapter::DidFail(uint64_t,
...
void VideoURLLoaderImpl::ClientAdapter::DidFailRedirectCheck(uint64_t identifier) {
  DidFail(identifier, ResourceError::Failure(NullURL()));
}

void VideoURLLoaderImpl::ClientAdapter::EnableErrorNotifications() {
...
void VideoURLLoaderImpl::ClientAdapter::NotifyError(TimerBase* timer) {
...
class VideoURLLoaderImpl::FrameProxy final : public GarbageCollected<FrameProxy> {
 public:
  FrameProxy(LocalFrame* local_frame) : local_frame_(local_frame) {}

  void Trace(Visitor* visitor) const { visitor->Trace(local_frame_); }

  void NotifyFinished(base::WeakPtr<VideoURLLoaderImpl> loader) {
    if (local_frame_) {
      local_frame_->NotifyFinished(loader);
    }
  }
  ...
  void RegisterUrlLoader(base::WeakPtr<VideoURLLoaderImpl> loader,
    ...
    if (local_frame_) {
      local_frame_->RegisterUrlLoader(loader, id, start, std::move(request),
                                      client);
    }
  }
...
class VideoURLLoaderImpl::Observer final
    : public GarbageCollected<Observer>,
      public ExecutionContextLifecycleObserver {
 public:
  Observer(VideoURLLoaderImpl* parent, ExecutionContext* context)
...
VideoURLLoaderImpl::VideoURLLoaderImpl(ExecutionContext* context,
    ...
    : client_(nullptr),
      ...
      frame_(MakeGarbageCollected<FrameProxy>(frame)) {}

VideoURLLoaderImpl::~VideoURLLoaderImpl() {
  Cancel();
}

void VideoURLLoaderImpl::NotifyFinished() {
  if (frame_) {
    frame_->NotifyFinished(weak_factory_.GetWeakPtr());
  }
}

void VideoURLLoaderImpl::Load(const WebURLRequest& request,
                           base::WeakPtr<WebAssociatedURLLoaderClient> client,
                           std::string id,
                           int64_t start) {
  DCHECK(client);
  ...
  frame_->RegisterUrlLoader(weak_factory_.GetWeakPtr(), id, start,
                            std::move(new_request), client);
}

void VideoURLLoaderImpl::LoadAsynchronously(const WebURLRequest& request,
                                         WebAssociatedURLLoaderClient* client) {
  DCHECK(!client_);
  DCHECK(!loader_);
  ...
  if (options_.untrusted_http) {
    ...
    allow_load = IsValidHTTPToken(method) &&
...
void VideoURLLoaderImpl::Cancel() {
  DisposeObserver();
  CancelLoader();
  ReleaseClient();
  frame_ = nullptr;
...
void VideoURLLoaderImpl::ClientAdapterDone() {
  DisposeObserver();
  ReleaseClient();
}

void VideoURLLoaderImpl::CancelLoader() {
  if (!client_adapter_) {
    return;
  }
...
void VideoURLLoaderImpl::SetDefersLoading(bool defers_loading) {
  if (loader_) {
    loader_->SetDefersLoading(defers_loading);
  }
}

void VideoURLLoaderImpl::SetLoadingTaskRunner(base::SingleThreadTaskRunner*) {
  // TODO(alexclarke): Maybe support this one day if it proves worthwhile.
}

void VideoURLLoaderImpl::ContextDestroyed() {
  DisposeObserver();
  CancelLoader();
...
void VideoURLLoaderImpl::DisposeObserver() {
  if (!observer_) {
    return;
  }
