...
// X509CertChainVerify
bool GetTrustAnchorsForHostName_stub(const std::string& hostname,
                                     std::vector<std::string>& certs) {
  base::FilePath path = GetTestCertsPath();
  certs.emplace_back(path.value());
  certs.emplace_back(path.AppendASCII(hostname).value());
  base::FilePath test_cert_dir;
  if (!base::GetCurrentDirectory(&test_cert_dir) || test_cert_dir.empty()) {
    return true;
  }
  const std::string net_test_tmp_dir = "tmp/net_test_tmp";
  test_cert_dir = test_cert_dir.Append(net_test_tmp_dir);
  certs.emplace_back(test_cert_dir.value());
  return true;
}

TEST_F(CertVerifyProcOHOSTest, CertVerify) {
  std::vector<std::string> cert_bytes;
  std::vector<std::string> verified_chain;
  auto [leaf, intermediate, root] = CertBuilder::CreateSimpleChain3();

  GetChainDEREncodedBytes(leaf->GetX509Certificate().get(), &cert_bytes);
  int result = CertVerify(cert_bytes, &verified_chain, kHostname);
  EXPECT_EQ(X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY, result);

  cert_bytes.emplace_back(kInvalidCertificatePEM);
  result = CertVerify(cert_bytes, &verified_chain, kHostname);
  EXPECT_EQ(X509_V_ERR_UNSPECIFIED, result);
}
...
TEST_F(CertVerifyProcOHOSTest, SetCertStatus) {
  ...
  // verify_result.cert_status = 0;
  // ASSERT_NO_FATAL_FAILURE(SetCertStatus(
  //     X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT + 100, &verify_result));
...
TEST_F(CertVerifyProcOHOSTest,
  ...
  certs.clear();
  ConvertToParsedCertificates({}, errors, certs);
