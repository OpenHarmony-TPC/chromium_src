...
// Based on direct_shared_image_video_provider.h originally written by
...
std::unique_ptr<PooledSharedImageVideoProvider> PooledSharedImageVideoProvider::Create(
...
void PooledSharedImageVideoProvider::RequestImage(ImageReadyCB cb,
  ...
  if (!pool_.empty()) {
    ...
    // liberato: See if skipping the return if |pool_| is now empty is
...
void PooledSharedImageVideoProvider::ProcessFreePooledImage(
  ...
  if (pending_requests_.size()) {
    ...
    if (pooled_image->spec == front.spec) {
      ...
      // liberato: Don't copy fields this way.
      ImageRecord record;
