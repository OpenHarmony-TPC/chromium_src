...
def BuildFFmpeg(target_os, target_arch, host_os, host_arch, parallel_jobs,
    ...
    # Anonymize OHOS code path.
    post_make_rewrites += [(CHROMIUM_ROOT_DIR + '/*', '')]
...
def ConfigureAndBuild(target_arch, target_os, host_os, host_arch,
    ...
    configure_flags['Common'].extend([
        ...
        # Enable playing avi, ts and flv files in OHOS OS.
        '--enable-decoder=mpeg4',
        '--enable-parser=h263,mpeg4video',
        '--enable-demuxer=avi',
        '--enable-demuxer=flv,live_flv,mpegts',
    ...
    def config_ohos():
        _ohos_sysroot_dir = os.path.join(CHROMIUM_ROOT_DIR, 'third_party', 'ohos_ndk', 'sysroot')
        _ohos_llvm_dir = os.path.join(CHROMIUM_ROOT_DIR, 'third_party', 'ohos_ndk', 'toolchains',
            'llvm')
        _ohos_abi = 'arm'
        _ohos_abi_target = 'arm-linux-ohosmusl'
        if target_arch == 'arm64':
            _ohos_abi = 'aarch64'
            _ohos_abi_target = 'aarch64-linux-ohos'
        configure_flags['Common'].extend([
            '--enable-pic',
            '--cc=' + os.path.join(_ohos_llvm_dir, 'bin', 'clang'),
            '--cxx=' + os.path.join(_ohos_llvm_dir, 'bin', 'clang++'),
            '--ld=' + os.path.join(_ohos_llvm_dir, 'bin', 'clang'),
            '--ar=' + os.path.join(_ohos_llvm_dir, 'bin', 'llvm-ar'),
            '--nm=' + os.path.join(_ohos_llvm_dir, 'bin', 'llvm-nm'),
            '--ranlib=' + os.path.join(_ohos_llvm_dir, 'bin', 'llvm-ranlib'),
            '--strip=' + os.path.join(_ohos_llvm_dir, 'bin', 'llvm-strip'),
            '--enable-cross-compile',
            '--sysroot=' + _ohos_sysroot_dir,
            '--extra-cflags=--target=' + _ohos_abi_target,
            '--extra-ldflags=--target=' + _ohos_abi_target,
            '--extra-ldflags=-L' + os.path.join(_ohos_sysroot_dir, 'usr', 'lib',
                '%s-linux-ohos' % _ohos_abi),
            '--extra-cflags=-I' + os.path.join(_ohos_sysroot_dir, 'usr', 'include'),
            '--extra-cflags=-I' + os.path.join(CHROMIUM_ROOT_DIR, 'ohos_ndk',
                'toolchains', 'llvm', 'include', 'libcxx-ohos', 'include', 'c++', 'v1'),
        ])
