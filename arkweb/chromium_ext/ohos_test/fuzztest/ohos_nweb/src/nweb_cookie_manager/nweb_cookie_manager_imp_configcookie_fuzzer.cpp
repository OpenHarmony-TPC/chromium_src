...
bool NWebCookieManagerImplConfigCookie(const uint8_t* data, size_t size) {
  ...
  cookieMgr->ConfigCookie(url, value, nweb_value_callback);
  return true;
}
}  // namespace OHOS::NWeb
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if (init) {
    OHOS::NWeb::NWebCookieManagerImplConfigCookie(data, size);
  }
  return 0;
}
