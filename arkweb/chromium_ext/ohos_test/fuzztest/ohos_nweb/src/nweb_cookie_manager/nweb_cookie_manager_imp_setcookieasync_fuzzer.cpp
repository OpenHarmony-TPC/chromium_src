...
bool NWebCookieManagerImplSetCookieAsync(const uint8_t* data, size_t size) {
  ...
  std::shared_ptr<NiceMock<MockNWebLongValueCallback>> nweb_value_callback =
      std::make_shared<NiceMock<MockNWebLongValueCallback>>();
  cookieMgr->SetCookieAsync(url, value, incognito_mode, includeHttpOnly,
                            nweb_value_callback);
  return true;
}
}  // namespace OHOS::NWeb
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if (init) {
    OHOS::NWeb::NWebCookieManagerImplSetCookieAsync(data, size);
  }
  return 0;
}
