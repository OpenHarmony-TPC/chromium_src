...
// Based on font_unique_name_lookup.cc originally written by
...
static const char* const kOhosFontPaths[] = {"/system/fonts"};
...
bool IsRelevantNameRecord(const FT_SfntName& sfnt_name) {
  ...
  // Since we can assume Ohos system fonts contain an English name,
  // continue here.
  ...
  base::ThreadPool::PostTask(FROM_HERE,
                             ...
                             base::BindOnce(
                                 [](FontUniqueNameLookup* instance) {
                                   ...
                                   // size, which it doesn't if the
                                   // LoadFromFile() failed. If it doesn't have
                                   // a size, the table is rebuild by calling
                                   // UpdateTable().
