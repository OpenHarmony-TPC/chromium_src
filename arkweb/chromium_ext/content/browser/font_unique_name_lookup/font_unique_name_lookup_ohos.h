...
// Based on font_unique_name_lookup.h originally written by
...
// it. The protobuf can be persisted to disk to the Ohos cache directory, and
...
class CONTENT_EXPORT FontUniqueNameLookup {
 public:
  ...
  // (i.e. if there was an Ohos firmware update or no cached one existed)
  // from the standard Ohos font directories, and writing the updated lookup
  // table back to file.
  ...
  // to access the Ohos build fingerprint.
  ...
  // If an Ohos firmware update was detected by checking
 ...
 private:
  // return the actual platform's Ohos build fingerprint.
  std::string GetOhosBuildFingerprint() const;

  // enumerate font files in the the Ohos platform font directories.
