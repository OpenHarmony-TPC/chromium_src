...
void MediaStreamManagerExt::StopScreenCapture(int32_t nweb_id,
  ...
  if(!BrowserThread::CurrentlyOn(BrowserThread::IO)){
    ...
    io_task_runner->PostTask(
      FROM_HERE,
      base::BindOnce(&MediaStreamManagerExt::StopScreenCapture,weak_factory_.GetWeakPtr(),nweb_id,session_id));
      return;
  }
...
void MediaStreamManagerExt::SetScreenCapturePickerShow() {
  if(!BrowserThread::CurrentlyOn(BrowserThread::IO)){
    ...
    io_task_runner->PostTask(
      FROM_HERE,
      base::BindOnce(&MediaStreamManagerExt::SetScreenCapturePickerShow,weak_factory_.GetWeakPtr()));
      return;
  }
...
void MediaStreamManagerExt::DisableSessionReuse() {
  if(!BrowserThread::CurrentlyOn(BrowserThread::IO)){
    ...
    io_task_runner->PostTask(
      FROM_HERE,
      base::BindOnce(&MediaStreamManagerExt::DisableSessionReuse,weak_factory_.GetWeakPtr()));
      return;
  }
