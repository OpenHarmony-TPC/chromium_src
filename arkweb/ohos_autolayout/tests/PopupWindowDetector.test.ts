...
jest.mock("../src/Framework/Utils/Utils", () => ({
  __esModule: true,
  default: {
    getScreenAreaRatio: (node: Element) => getScreenAreaRatioMock(node),
    isBackgroundSemiTransparent: (style: CSSStyleDeclaration) =>
      isBackgroundSemiTransparentMock(style),
    hasCloseButton: (element: Element | null) => hasCloseButtonMock(element),
    visualFilter: (node: Element) => visualFilterMock(node),
    isElementVisibleInViewPort: (el: Element | null) =>
      isElementVisibleInViewPortMock(el),
    zIndexToNumber: (value: string) => zIndexToNumberMock(value),
    findSiblingContent: (mask: Element) => findSiblingContentMock(mask),
    getElementDepth: (element: Element) => getElementDepthMock(element),
    isColorTransparent: (color: string) => isColorTransparentMock(color),
    isColorSemiTransparent: (color: string) =>
...
jest.mock("../src/Framework/Utils/LayoutUtils", () => ({
  __esModule: true,
  default: {
    compareZIndex: (a: Element, b: Element) => compareZIndexMock(a, b),
    analyzeComputedBoxShadow: (value: string) =>
      analyzeComputedBoxShadowMock(value),
    hasElementSiblings: (element: HTMLElement) =>
...
jest.mock("../src/Framework/Popup/PopupRecog", () => ({
  PopupRecog: {
    predictIsPopup: (info: PopupInfo) => predictIsPopupMock(info),
  },
}));
...
describe("PopupWindowDetector", () => {
  describe("Component Visibility Detection", () => {
    it("should scan descendants and respect transparent nodes in isStickyComponentVisiable", () => {
      ...
      expect(
        (PopupWindowDetector as any).isStickyComponentVisiable(root)
      ).toBe(true);
    });
    ...
    it("should handle visibility, opacity and width deviation in isElementQualified", () => {
      ...
      expect(
        (PopupWindowDetector as any).isElementQualified(
          element,
  ...
  describe("Mask Detection", () => {
    ...
    expect(
      (PopupWindowDetector as any).isPotentialMask(
        mask,
  ...
  describe("Sticky Element Detection", () => {
    ...
    const resultTop = (PopupWindowDetector as any).checkStickyElement(
      sticky,
    ...
    const resultBottom = (PopupWindowDetector as any).checkStickyElement(
      sticky,
      window.getComputedStyle(sticky),
      null,
      null
  ...
  describe("Element Comparison and Ordering", () => {
    ...
    expect(
      (PopupWindowDetector as any).isFirstElementOnTop(a, b)
    ).toBe(true);
    ...
    it("should detect older siblings accurately in isPreviousElementSibling", () => {
      ...
      expect(
        (PopupWindowDetector as any).isPreviousElementSibling(current, prev)
      ).toBe(true);
      
      expect(
        (PopupWindowDetector as any).isPreviousElementSibling(prev, current)
      ).toBe(false);
    });
  });
  ...
  describe("Content Finding", () => {
    ...
    const result = (PopupWindowDetector as any).findBestCandidateInSibling(
      sibling,
      mask,
      1
    );
    ...
    const [candidate, root] = (
      PopupWindowDetector as any
    ).findBestSiblingContent(mask);
    ...
    expect(
      (PopupWindowDetector as any).findBestDescendantContent(mask)
    ).toBe(childB);
    ...
    expect(
      (PopupWindowDetector as any).findPositionedAncestor(child)
    ).toBe(parent);
    });
  });
  ...
  describe("Popup Structure Analysis", () => {
    ...
    const resultSibling = (PopupWindowDetector as any).determinePopupStructure(
      mask
    );
    ...
    const resultDesc = (PopupWindowDetector as any).determinePopupStructure(
      maskDesc
    );
    ...
    const result = (PopupWindowDetector as any).identifyPopupFromMask(mask);
    ...
    it("should aggregate metrics from DOM in createPopupInfo", () => {
      ...
      const info = (PopupWindowDetector as any).createPopupInfo(
        root,
        mask,
        content,
        PopupType.A
  ...
  describe("Filtering and Prediction", () => {
    ...
    const result = (PopupWindowDetector as any).filterNestedPopups([
      parentInfo,
      childInfo,
    ]);
    ...
    const resultSame = (PopupWindowDetector as any).filterNestedPopups([
      infoA,
      infoB,
    ]);
    ...
    expect(
      (PopupWindowDetector as any).filterByPrediction([infoFast, infoPredict])
    ).toEqual([infoFast, infoPredict]);
    
    predictionMap.set(infoPredict.root_node, false);
    
    expect(
      (PopupWindowDetector as any).filterByPrediction([infoPredict])
    ).toEqual([]);
    });
    ...
    (PopupWindowDetector as any).updateStickyHeights(popup, {
  ...
  describe("Pipeline Integration", () => {
    ...
    const result = (PopupWindowDetector as any).findPopupsInternal(potential);
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("isStickyComponentVisiable returns true only when a visible descendant matches width", () => {
      ...
      expect((PopupWindowDetector as any).isStickyComponentVisiable(root)).toBe(
        true
      );
    ...
    it("checkStickyElement returns original values when candidate conditions fail", () => {
      ...
      const result = (PopupWindowDetector as any).checkStickyElement(
        el,
        window.getComputedStyle(el),
        null,
        null
    ...
    it("isFirstElementOnTop gracefully handles non-overlap and missing top element", () => {
      ...
      expect((PopupWindowDetector as any).isFirstElementOnTop(elA, elB)).toBe(
        false
      );
    ...
    it("determinePopupStructure returns null when no valid sibling or descendant is found", () => {
      ...
      expect(
        (PopupWindowDetector as any).determinePopupStructure(mask)
      ).toBeNull();
    });
    ...
    it("findDescendantBasedPopup returns null when no descendant passes filters", () => {
      ...
      expect(
        (PopupWindowDetector as any).findDescendantBasedPopup(mask)
      ).toBeNull();
    });
    ...
    it("isPreviousElementSibling returns false for unrelated nodes", () => {
      ...
      expect((PopupWindowDetector as any).isPreviousElementSibling(a, b)).toBe(
        false
      );
