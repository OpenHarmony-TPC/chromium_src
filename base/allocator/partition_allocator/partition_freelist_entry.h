...
ALWAYS_INLINE static uintptr_t GetRandomValue()
{
    uint64_t high = RandomValue();
    uint64_t low = RandomValue();
...
class PartitionFreelistEntry {
 ...
 private:
  ...
  ALWAYS_INLINE static void* Transform(void* ptr) {
    ...
    return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(ptr) ^ g_freelist_cookie);
#else
