...
void Dangling() {
  ...
  [[maybe_unused]] auto a = span<const int>(std::to_array({1, 2, 3}));     // expected-error@181 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@181  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}

  [[maybe_unused]] auto b = span<const int, 3>(std::to_array({1, 2, 3}));  // expected-error@184 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@184  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}

  // Range destroyed at the end of the full expression.
  [[maybe_unused]] auto c = span<const int>(std::vector<int>({1, 2, 3}));     // expected-error@188 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@188  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}

  [[maybe_unused]] auto d = span<const int, 3>(std::vector<int>({1, 2, 3}));  // expected-error@191 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@191  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}
  
  // Here the `std::string` is an lvalue, but the `std::vector`s that copy its
  // data aren't.
  std::string str = "123";
  [[maybe_unused]] auto e =
      span<const char>(std::vector<char>(str.begin(), str.end()));  // expected-error@198 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@198  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}
  [[maybe_unused]] auto f =
      span<const char, 3>(std::vector<char>(str.begin(), str.end()));  // expected-error@201 {{object backing the pointer will be destroyed at the end of the full-expression}}
  // expected-error-re@201  {{temporary whose address is used as value of local variable {{.*}}will be destroyed at the end of the full-expression}}
