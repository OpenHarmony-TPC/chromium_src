...
static std::unordered_map<std::string, std::pair<FieldInfo, FieldInfo>> tempVector;

static void BindMethodAndField(aki::ClassBase *xlass)
{
...
static napi_value Init(napi_env env, napi_value exports)
{
    ...
    for (auto& xlass : aki::Binding::GetClassList()) {
        ...
        for (auto extendClassName : xlassName) {
            ...
            for (auto &extendClass : aki::Binding::GetClassList()) {
                if (strcmp(extendClass->GetName(), extendClassName) == 0) {
                    BindMethodAndField(extendClass);
                }
            }
        }
        AKI_DLOG(DEBUG) << "begin to bind method";
        BindMethodAndField(xlass);
        napi_value cons;
        // 存储类级别的cons对象的媒介
        ...
        napi_status status;
        status = napi_define_class(env, xlass->GetName(), NAPI_AUTO_LENGTH, wrapper, commonInfo, properties.size(),
                                   properties.data(), &cons);
        AKI_DCHECK(status == napi_ok);
