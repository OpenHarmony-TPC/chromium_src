...
public:
    ...
    operator uint32_t() { return GetUInt(); }
    ...
    virtual uint32_t GetUInt() const { return std::numeric_limits<uint32_t>::quiet_NaN(); }

    virtual uint64_t GetUInt64() const { return std::numeric_limits<uint64_t>::quiet_NaN(); }
    ...
    virtual ArrayBuffer GetArrayBuffer() const
    ...
    template<typename K, typename T>
    std::unordered_map<K, T>& HashMapReference ()
    {
        typedef std::unordered_map<K, T> M;
        return *(reinterpret_cast<M*>(GetDataReference()));
    }

    template<typename K, typename T>
    std::unordered_set<K, T>& HashSetReference ()
    {
        typedef std::unordered_set<K, T> M;
        return *(reinterpret_cast<M*>(GetDataReference()));
    }
    ...
    template<typename T>
    std::optional<T>& OptionalReference ()
    {
        typedef std::optional<T> O;
        return *(reinterpret_cast<O*>(GetDataReference()));
    }
