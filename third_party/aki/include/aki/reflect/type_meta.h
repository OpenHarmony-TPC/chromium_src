...
template<typename T>
#if USING_CXX_STANDARD_11
    constexpr bool isArray<std::vector<T>> = true;
#else
...
template<typename M>
#if USING_CXX_STANDARD_11
    constexpr bool isFunction<std::function<M>> = true;
#else
...
#if USING_CXX_STANDARD_11
    TypeMeta(const TypeMeta&) = default;
    TypeMeta(TypeMeta&&) = default;
    TypeMeta& operator=(const TypeMeta&) = default;
    TypeMeta& operator=(TypeMeta&&) = default;
#else
private:
