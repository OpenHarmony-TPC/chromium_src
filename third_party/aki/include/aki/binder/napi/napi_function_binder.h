...
private:
    ...
    static napi_value SafetyWrapper(napi_env& env, napi_callback_info info, std::index_sequence<Index...>)
    {
        ...
        return WrapperForward(env, bindInfo, result.data(), std::is_void<R>(),
...
#if USING_CXX_STANDARD_11
    template<size_t... Index>
    static napi_value WrapperForward(napi_env& env, aki::BindInfo* bindInfo, internal::Value* values[],
        std::true_type, std::index_sequence<Index...>)
    {
        FUNCTION_DTRACE();
    ...
    template<size_t... Index>
    static napi_value WrapperForward(napi_env& env, aki::BindInfo* bindInfo, internal::Value* values[],
        std::false_type, std::index_sequence<Index...>)
    {
        FUNCTION_DTRACE();
        return NapiValue<R>::ToNapiValue(env, ParentType::InnerWrapper(bindInfo,
            NapiValueTrait<P>::Cast(*values[Index])...));
    }
#else
    template<size_t... Index>
