...
void PassingValueAsArrayBuffer(aki::Value value)
{
    int8_t *data = reinterpret_cast<int8_t *>(value.As<aki::ArrayBuffer>().GetData());
    data[4] = 4;
    data[5] = 5;
    data[6] = 6;
    data[7] = 7;
...
void DoSomeWork(void)
{
    Handle handle;
    bool flag = false;
    if (nullptr == handle.value.GetHandle() && \
        !handle.value.IsNumber() &&  \
        !handle.value.IsUndefined() && \
        !handle.value.IsNull() && \
        !handle.value.IsBool() && \
        !handle.value.IsString() && \
        !handle.value.IsObject() && \
        !handle.value.IsArray() && \
        !handle.value.IsFunction()) {
        flag = true;
     }
...
bool PassingVoidReturnBool(void)
{
    /* 测试在非JS线程aki::Value默认构造时的异常处理 */
...
JSBIND_GLOBAL()
{
    ...
    JSBIND_FUNCTION(PassingValueAsArrayBuffer);
    JSBIND_FUNCTION(PassingVoidReturnBool);
}
