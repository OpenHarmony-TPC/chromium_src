...
void MediaControlScrubbingMessageElement::SetMsgSpan() {
  ...
  shadow_root->ParserAppendChild(style);
  current_time_span_ = MediaControlElementsHelper::CreateSpanWithId(AtomicString("messageCurrentTime"), shadow_root);
  duration_time_span_ = MediaControlElementsHelper::CreateSpanWithId(AtomicString("messageDurationTime"), shadow_root);
  duration_progress_ = MediaControlElementsHelper::CreateProgressWithId(AtomicString("durationProgress"), shadow_root);
}

void MediaControlScrubbingMessageElement::updateScrubbingMsg(bool wanted) {
  if (wanted) {
    const double current = IsHLSLive() ? 0 : MediaElement().currentTime();
...
bool MediaControlScrubbingMessageElement::IsHLSLive() const {
...
void MediaControlScrubbingMessageElement::Trace(Visitor* visitor) const {
  visitor->Trace(current_time_span_);
  visitor->Trace(duration_time_span_);
  visitor->Trace(duration_progress_);
